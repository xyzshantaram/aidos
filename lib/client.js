...
/* #137: the queue's state groups. The heading carries the state, so the
   strips below it drop the parenthesised copy they used to repeat. */
.aidos-queue-group + .aidos-queue-group {
  /* Separation between sections, not around every row: the groups are the
     structure, and spacing is what says so without a rule line. */
  margin-top: 14px;
}

.aidos-queue-group-heading {
  display: flex;
  align-items: center;
  gap: 6px;
  margin: 0 0 6px 2px;
  /* Deliberately quiet: this is a label for the rows beneath it, not a
     title competing with them. Same size as the strip meta. */
  font-size: 11px;
  font-weight: 600;
  text-transform: uppercase;
  letter-spacing: 0.04em;
...
.aidos-queue-group-count {
  /* How many asks are in this section, so the size of each piece of work
     is visible before you start reading rows. */
  font-weight: 400;
...
var QUEUE_GROUP_ORDER = ["open", "in_progress", "awaiting_verification"];
var QUEUE_GROUP_LABELS = {
  open: "Sign off",
  in_progress: "Approve",
  awaiting_verification: "Verify",
  other: "Other"
};
function groupQueueByState(entries) {
  const groups = [];
  ...
  if (rest.length > 0) {
    groups.push({ state: "other", label: QUEUE_GROUP_LABELS.other, entries: rest });
  }
  return groups;
}
...
function QueuePanel(props) {
  ...
  ))), unmatched.length > 0 ? /* @__PURE__ */ import_react25.default.createElement("ul", { className: "aidos-queue-unmatched" }, unmatched.map((row) => /* @__PURE__ */ import_react25.default.createElement("li", { key: row.nomination.id }, "The agent suggested " + row.nomination.actionId + " but it is not shown: " + row.reason))) : null, groupQueueByState(visible).map((group) => /* @__PURE__ */ import_react25.default.createElement("div", { className: "aidos-queue-group", key: group.state }, /* @__PURE__ */ import_react25.default.createElement("h3", { className: "aidos-queue-group-heading" }, group.label, /* @__PURE__ */ import_react25.default.createElement("span", { className: "aidos-queue-group-count" }, group.entries.length)), /* @__PURE__ */ import_react25.default.createElement("ul", { className: "aidos-ticket-strips" }, group.entries.map((entry) => /* @__PURE__ */ import_react25.default.createElement(
    TicketStrip,
    {
      showState: false,
      key: entryKey(entry),

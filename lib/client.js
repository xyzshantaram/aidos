...
/* \u2500\u2500 #82: the scratch tools' conversation rows \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
   Ported in SHAPE from tool-render (leading indicator, tool name, path
   summary, collapsible body, error on the row) but built on aidos's own
   tokens, because #72 requires aidos to depend on nothing external and
   tool-render is a separate plugin that may not be mounted.

   Before this the scratch tools rendered a raw JSON envelope, so a scratch
   read looked like a data dump beside a builtin read's clean row. */
.aidos-toolrow {
  border: 1px solid var(--border-subtle);
  border-radius: var(--radius-sm);
  background: var(--surface);
...
.aidos-toolrow-is-error {
  border-color: color-mix(in srgb, var(--verdict-fail) 45%, transparent);
}
...
/* A row with nothing to expand is not a button: no pointer, no hover, so the
   affordance never lies about what clicking will do. */
.aidos-toolrow-head:disabled {
  cursor: default;
}

.aidos-toolrow-head:not(:disabled):hover {
  background: var(--surface-hover);
}
...
/* The summary is the part that differs between rows, so it takes the space
   and truncates rather than wrapping -- a wrapped path makes the column
   ragged and hides the next row's title. */
...
// src/client/scratch-rows.tsx
function StateMark({ state, open }) {
  if (open) return /* @__PURE__ */ import_react28.default.createElement("span", { className: "aidos-toolrow-mark" }, "\u25BE");
  if (state === "error") return /* @__PURE__ */ import_react28.default.createElement("span", { className: "aidos-toolrow-mark aidos-toolrow-error" }, "\u25CF");
  if (state === "stopped") return /* @__PURE__ */ import_react28.default.createElement("span", { className: "aidos-toolrow-mark aidos-toolrow-stopped" }, "\u25CF");
  if (state === "running") return /* @__PURE__ */ import_react28.default.createElement("span", { className: "aidos-toolrow-mark" }, "\u25CC");
  return /* @__PURE__ */ import_react28.default.createElement("span", { className: "aidos-toolrow-mark" }, "\u25B8");
}
function ScratchRow({ title, summary, state, body, errorSummary }) {
  ...
  const shown = state === "error" && errorSummary !== void 0 ? errorSummary : summary;
  return /* @__PURE__ */ import_react28.default.createElement("div", { className: "aidos-toolrow" + (state === "error" ? " aidos-toolrow-is-error" : "") }, /* @__PURE__ */ import_react28.default.createElement(
    "button",
    {
      type: "button",
    ...
    /* @__PURE__ */ import_react28.default.createElement(StateMark, { state, open }),
    /* @__PURE__ */ import_react28.default.createElement("span", { className: "aidos-toolrow-title" }, title),
    /* @__PURE__ */ import_react28.default.createElement(
      "span",
      {
        className: "aidos-toolrow-summary" + (state === "error" && errorSummary !== void 0 ? " aidos-toolrow-error" : ""),
        title: shown
      },
      shown
    )
...
function bodyOf(text, isError) {
  ...
  return /* @__PURE__ */ import_react28.default.createElement("pre", { className: "aidos-toolrow-pre" + (isError ? " aidos-toolrow-error" : "") }, text);
}

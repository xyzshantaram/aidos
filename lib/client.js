...
/*
 * #93: the queue's action buttons, same size, right-aligned.
 *
 * FOUR attempts used a GRID and each broke the alignment in a new way --
 * "lost the right alignment", "still broken", then "all different sizes and
 * collapse to the left". The container's \`margin-left: auto\` inside a flex
 * strip was the thing that always worked; every failure came from replacing
 * that layout rather than adding to it.
 *
 * So: no grid, no display override. The container keeps its flex layout
 * untouched, and the ONLY addition is a fixed width on the primary button.
 * That is the whole requirement -- "all the same size" -- expressed as the
 * smallest change from a known-good baseline.
 *
 * Dismiss keeps its natural width and renders FIRST (see queue-panel.tsx),
 * so the primary button lands at the same place on every row without
 * reserving an empty column, which is what produced the gap beside "Sign
 * off" in the previous attempt.
 */
...
.aidos-queue .aidos-btn-primary {
  /* Sized to the longest label ("Review request"). Fixed, not min-width, so
     a short label like "Verify" cannot shrink and leave the column ragged. */
  width: 7.5rem;
  flex: none;
  justify-content: center;
  min-height: 28px;
...
@media (max-width: 640px) {
  /* A fixed width would squeeze the label on a narrow screen rather than
     aligning it, so the primary button goes back to its content size. */
  .aidos-queue .aidos-btn-primary {
    width: auto;
    min-width: 5rem;
  }
...
function AttachEvidenceRow(props) {
  ...
  const body = state !== "error" && kind !== void 0 ? /* @__PURE__ */ import_react29.default.createElement("ul", { className: "aidos-evidence-strips" }, /* @__PURE__ */ import_react29.default.createElement(
    EvidenceStrip,
    {
      row: {
...
function GetTicketsRow(props) {
  ...
  return /* @__PURE__ */ import_react29.default.createElement(AidosRow, { title: "Read the board", summary, state, errorSummary });
}
var AIDOS_ROWS = [

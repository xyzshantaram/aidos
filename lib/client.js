...
function unmatchedNominations(tickets, evidenceKindsOf, nominations) {
  ...
  for (const nomination of nominations) {
    ...
    out.push({
      nomination,
      reason: !onBoard ? "#" + key + " is not on this board (it may belong to another session)" : "#" + key + " has no available " + nomination.actionId + " action right now"
    });
  }
  return out;
}
  ...
  const refreshNominations = import_react27.default.useCallback(
    function() {
      ...
      void callAidosRemote("actionNominations", {}, sessionId).then((rows) => {
        ...
        setNominations([]);
        setQueueError(
          "nominations: " + (error2 instanceof Error ? error2.message : String(error2))
        );
      });
      void callAidosRemote("pendingApprovals", {}, sessionId).then((rows) => {
        ...
        setApprovals([]);
        setQueueError(
          "approvals: " + (error2 instanceof Error ? error2.message : String(error2))
        );
      });
    },
    [sessionId]

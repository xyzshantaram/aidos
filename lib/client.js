 ...
 */
/*
 * #93: the caller's action buttons sit on a grid so every row's primary
 * button is the same size, while the CONTAINER stays flex so it keeps the
 * right alignment it always had.
 *
 * The first attempt made the whole actions container a 3-column grid. That
 * could not align reliably, because the container mixes the strip's own
 * pop-out affordance with the caller's buttons and its child COUNT varies --
 * the icon may be absent, a Dismiss may or may not follow. A fixed template
 * misaligns the moment either changes, which is what the user saw twice.
 *
 * Split by concern: flex here, grid on the action group only.
 */
.aidos-queue .aidos-ticket-strip-action-group {
  display: grid;
  grid-template-columns: var(--queue-action-w) var(--queue-dismiss-w);
  gap: 6px;
  align-items: center;
}

.aidos-queue {
  /* Sized once to the longest label ("Review request"), on the CONTAINER, so
     the columns do not resize per row -- that resizing is the raggedness
     being removed. */
  --queue-action-w: 8.5rem;
  --queue-dismiss-w: 5rem;
}

.aidos-queue .aidos-ticket-strip-action-group .aidos-btn {
  /* Full width WITHIN its column: the column decides the size, not the label. */
  width: 100%;
  justify-content: center;
  min-height: 28px;
}
...
@media (max-width: 640px) {
  /* Share the space rather than holding fixed widths: on a narrow screen
     fixed columns squeeze the labels instead of aligning them. */
  .aidos-queue .aidos-ticket-strip-action-group {
    grid-template-columns: 1fr 1fr;
  }
}
...
/* \u2500\u2500 #82: the scratch tools' conversation rows \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
   MATCHED to tool-render rather than approximated. The first attempt built a
   bordered card with its own sizing, which read as a different widget beside
   a builtin fs row -- the user's verdict was "not close enough".

   Two things make it actually match. First the STRUCTURE: a bare 1.5rem flex
   line with no border and no background, a 1rem leading slot, the tool name,
   a small dot separator, then the path; the body is a rounded code block
   below it. Second the TOKENS: tool-render uses the harness's own
   --dsw-alias-* design tokens, which are global to the page, so using them
   here is matching the app's design system rather than depending on another
   plugin. A fallback follows each one so the rows stay legible if a token is
   ever absent. */
.aidos-toolrow {
  display: flex;
  flex-direction: column;
...
.aidos-toolrow-line[data-expandable] {
  cursor: pointer;
}
...
.aidos-toolrow-leading {
  ...
  color: var(--dsw-alias-label-tertiary, var(--text-muted));
}

/* The error state enlarges the dot and colours the title, exactly as
   tool-render does: the row must read as failed without being opened. */
.aidos-toolrow-line[data-state="error"] .aidos-toolrow-leading {
  transform: scale(1.3);
  color: var(--dsw-alias-state-error-primary, var(--verdict-fail));
}

.aidos-toolrow-line[data-state="stopped"] .aidos-toolrow-leading {
  color: var(--dsw-alias-state-warning-primary, var(--state-awaiting));
}
...
.aidos-toolrow-line[data-state="error"] .aidos-toolrow-title {
  color: var(--dsw-alias-state-error-primary, var(--verdict-fail));
  font-weight: 500;
}

/* The dot between the name and the path. */
.aidos-toolrow-sep {
  flex: none;
  width: 0.125rem;
  height: 0.125rem;
  ...
  background: var(--dsw-alias-label-caption, var(--text-muted));
}
...
.aidos-toolrow-summary[data-error] {
  color: var(--dsw-alias-state-error-primary, var(--verdict-fail));
  font-weight: 500;
}
...
.aidos-toolrow-code {
  ...
  background: var(--dsw-alias-markdown-code-block, var(--surface-hover));
...
function TicketStrip(props) {
  ...
  ) : null, props.actions !== void 0 ? /* @__PURE__ */ import_react17.default.createElement("span", { className: "aidos-ticket-strip-action-group" }, props.actions) : null)));
}
...
function ScratchRow({ title, summary, state, body, errorSummary }) {
  ...
  return /* @__PURE__ */ import_react28.default.createElement("div", { className: "aidos-toolrow" }, /* @__PURE__ */ import_react28.default.createElement(
    "button",
    {
      type: "button",
    ...
    /* @__PURE__ */ import_react28.default.createElement("span", { className: "aidos-toolrow-leading", "aria-hidden": "true" }, /* @__PURE__ */ import_react28.default.createElement(Leading, { state, open })),
    /* @__PURE__ */ import_react28.default.createElement("span", { className: "aidos-toolrow-title" }, title),
    /* @__PURE__ */ import_react28.default.createElement("span", { className: "aidos-toolrow-sep", "aria-hidden": "true" }),
    /* @__PURE__ */ import_react28.default.createElement(
      "span",
      {
        className: "aidos-toolrow-summary",
        "data-error": showsError ? true : void 0,
        title: shown
      },
      shown
    )
  ), open ? /* @__PURE__ */ import_react28.default.createElement("div", { className: "aidos-toolrow-body" }, body) : null);
}
...
function bodyOf(text, isError) {
  ...
  return /* @__PURE__ */ import_react28.default.createElement("pre", { className: "aidos-toolrow-code", "data-error": isError ? true : void 0 }, text);
}
function ScratchReadRow(props) {
  const { state, summary, errorText: errorText2, errorSummary } = useRow(props, "Read");

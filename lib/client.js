...
`,e=e.substring(p.length+1),l=!0),!l){let k=this.rules.other.nextBulletRegex(y),I=this.rules.other.hrRegex(y),v=this.rules.other.fencesBeginRegex(y),M=this.rules.other.headingBeginRegex(y),F=this.rules.other.htmlBeginRegex(y),Q=this.rules.other.blockquoteBeginRegex(y);for(;e;){let A=e.split(`
`,1)[0],j;if(p=A,this.options.pedantic?(p=p.replace(this.rules.other.listReplaceNesting,"  "),j=p):j=p.replace(this.rules.other.tabCharGlobal,"    "),v.test(p)||M.test(p)||F.test(p)||Q.test(p)||k.test(p)||I.test(p))break;if(j.search(this.rules.other.nonSpaceChar)>=y||!p.trim())c+=`
`+j.slice(y);else{if(h||f.replace(this.rules.other.tabCharGlobal,"    ").search(this.rules.other.nonSpaceChar)>=4||v.test(f)||M.test(f)||I.test(f))break;c+=`
`+p}h=!p.trim(),d+=A+`

  ...
  if (entry.actionId === "signoff") {
    return [
      confirm,
      {
        ...
        prompt: "Signoff alone grants write access to nothing. Name the files and directories the agent may write while it works \u2014 or leave this empty and scope them later.",
  ...
  async function performQueueAction(entry, outcome) {
    ...
    try {
      ...
      if (entry.actionId === "signoff") {
        ...
        const paths = pathStep !== void 0 && pathStep.kind === "path-list" ? pathStep.paths : [];
        if (paths.length > 0) {
          try {
            await callAidosRemote("userGrantAllowlist", { ticketId, paths }, sessionId);
            showToast("Signed off \xB7 " + paths.length + " path(s) granted", "success");
          } catch (grantError) {
            const detail = grantError instanceof Error ? grantError.message : String(grantError);
            throw new Error(
              `#${ticketId} was signed off and moved to in_progress, but the allowlist was refused: ${detail} \u2014 the ticket is in progress with no write access; set the paths from its detail panel`
            );
          }
        } else {
          showToast("Signed off", "success");

...
// css-text:src/client/plan-meta.css
...
// css-text:src/client/vendor/tool-render/tool-render.css
...
function compareTicketTitles(a, b2) {
...
function compareTicketViews(a, b2, key = "confidence", descending = true) {
  const aHas = ticketHasCriteria(a);
  const bHas = ticketHasCriteria(b2);
  ...
  switch (key) {
    ...
    case "time":
      ...
      tiebreak = compareTicketTitles(a.title, b2.title);
      break;
    case "alpha":
      primary = compareTicketTitles(a.title, b2.title);
...
function filterTicketViews(views, filter = {}) {
  ...
  const tagSet = filter.tags !== void 0 && filter.tags.length > 0 ? new Set(filter.tags) : null;
  const out = [];
  ...
  out.sort(
    (a, b2) => compareTicketViews(a, b2, filter.sortKey ?? "confidence", filter.descending ?? true)
  );
  return out;
}
...
function boardKeyOf(ticket) {
  return boardKeyText(ticket);
}
function resolveDeepLinkRow(ref, rows) {
...
function hideRetiredTickets(tickets, evidenceByTicket) {
...
function filterTickets(tickets, filter) {
  return filterTicketViews(tickets, {
    ...
    projectIds: filter.projectIds ?? void 0,
...
function gateIsFailed(present, total, hasCriteriaValue) {
...
var knownWorkspaceLabels = /* @__PURE__ */ new Map();
function rememberWorkspaceLabel(workspaceKey, label) {
...
function tagCounts(tickets) {
  const counts2 = /* @__PURE__ */ new Map();
  for (const ticket of tickets) {
    for (const tag of ticket.tags ?? []) {
      counts2.set(tag, (counts2.get(tag) ?? 0) + 1);
    }
  }
  const out = [];
  for (const [tag, count] of counts2) {
    out.push({ tag, count });
  }
  out.sort((a, b2) => b2.count - a.count || (a.tag < b2.tag ? -1 : a.tag > b2.tag ? 1 : 0));
  return out;
}
...
function cloneAppliedState(state) {
  return {
    ...
    tags: state.tags === void 0 ? void 0 : [...state.tags]
  };
}
...
var lastRenderedSessionId = null;
var authoritativeSessionId = null;
...
var remountSuppressed = false;
var relabelPending = false;
function setRemountSuppressed(suppressed) {
...
function isModalOpen(sessionId, modal) {
  return openModals.get(sessionId)?.has(modal) === true;
}
function setModalOpen(sessionId, modal, open) {
  const current = openModals.get(sessionId);
  if (open) {
    if (current === void 0) openModals.set(sessionId, /* @__PURE__ */ new Set([modal]));
...
function getViewedEvidence(sessionId, ticketId) {
  return detailEntry(sessionId, ticketId, false)?.evidence ?? null;
}
function setViewedEvidence(sessionId, ticketId, row) {
  if (row !== null) {
    const entry2 = detailEntry(sessionId, ticketId, true);
    if (entry2 !== void 0) entry2.evidence = row;
    return;
  }
  const entry = detailEntry(sessionId, ticketId, false);
  if (entry === void 0) return;
...
function clearDetailModals(sessionId, ticketId) {
  const byTicket = detailModals.get(sessionId);
  if (byTicket === void 0) return;
  byTicket.delete(ticketId);
  if (byTicket.size === 0) detailModals.delete(sessionId);
}
var runningApprovals = /* @__PURE__ */ new Map();
function getRunningApproval(sessionId) {
  return runningApprovals.get(sessionId) ?? null;
}
function setRunningApproval(sessionId, key) {
...
var heldTickets = /* @__PURE__ */ new Map();
function getHeldTicket(sessionId) {
  return heldTickets.get(sessionId) ?? null;
}
function setHeldTicket(sessionId, ticket) {
  if (ticket === null) heldTickets.delete(sessionId);
  else heldTickets.set(sessionId, ticket);
}
function holdInput(sessionId, refValue) {
  return refValue ?? getHeldTicket(sessionId);
}
function recordResolution(sessionId, reason, ticket) {
...
function titleKey(sessionId, ticketId) {
  return sessionId + "\0" + String(ticketId);
}
function publishTicketTitles(sessionId, rows) {
  for (const row of rows) {
    if (row.foreign === true && row.sourceSessionId === void 0) continue;
    ticketTitles.set(titleKey(row.sourceSessionId ?? sessionId, row.id), row.title);
  }
}
function ticketTitle(sessionId, ticketId) {
...
var FILTER_APPLY_DELAY_MS = 1200;
function FilterApplyStatus(props) {
  if (props.pending) {
    return /* @__PURE__ */ import_react.default.createElement("span", { className: "aidos-filter-status", "aria-live": "polite" }, /* @__PURE__ */ import_react.default.createElement("span", { className: "aidos-merge-spinner" }), /* @__PURE__ */ import_react.default.createElement("span", null, "Filtering\u2026"));
  }
  if (props.dirty) {
    return /* @__PURE__ */ import_react.default.createElement("span", { className: "aidos-filter-status", "aria-live": "polite" }, /* @__PURE__ */ import_react.default.createElement("span", null, "Not applied yet"));
  }
  return /* @__PURE__ */ import_react.default.createElement("span", { className: "aidos-filter-status" });
}
function FilterPanel(props) {
  ...
  const applyRef = import_react.default.useRef(null);
  const [applyPending, setApplyPending] = import_react.default.useState(false);
  ...
  const searchSection = /* @__PURE__ */ import_react.default.createElement("div", { className: "aidos-panel-section" }, /* @__PURE__ */ import_react.default.createElement("div", { className: "aidos-panel-head" }, /* @__PURE__ */ import_react.default.createElement("h4", { className: "aidos-panel-title" }, "Search")), /* @__PURE__ */ import_react.default.createElement("div", { className: "aidos-search-box" }, /* @__PURE__ */ import_react.default.createElement(
    ...
    /* @__PURE__ */ import_react.default.createElement(
      "span",
      {
        ...
        style: { ["--chip-hue"]: idColor(fullTicketId(ticket)) },
  ...
  const actionRows = /* @__PURE__ */ import_react.default.createElement("div", { className: "aidos-actions-row" }, /* @__PURE__ */ import_react.default.createElement(FilterApplyStatus, { pending: applyPending, dirty }), /* @__PURE__ */ import_react.default.createElement("button", { className: "aidos-btn", onClick: reset }, "Reset"));
  ...
  const tagChips = availableTags.length === 0 ? null : /* @__PURE__ */ import_react.default.createElement("div", { className: "aidos-filter-chips", role: "group", "aria-label": "Tags" }, availableTags.map(({ tag, count }) => {
  ...
  return /* @__PURE__ */ import_react.default.createElement("div", { className: "aidos-filterbar" }, /* @__PURE__ */ import_react.default.createElement("div", { className: "aidos-filterbar-left" }, stateChips, tagChips, props.projects === void 0 ? null : props.projects.length === 0 ? null : /* @__PURE__ */ import_react.default.createElement(
    "select",
    {
  ...
  ))) : null), /* @__PURE__ */ import_react.default.createElement(FilterApplyStatus, { pending: applyPending, dirty }), /* @__PURE__ */ import_react.default.createElement("button", { className: "aidos-btn", onClick: reset }, "Reset")));
}
...
function UnretireIcon() {
  return /* @__PURE__ */ import_react3.default.createElement("svg", { ...iconProps() }, /* @__PURE__ */ import_react3.default.createElement("path", { d: "M6.5 10H2V2h8v4.5" }), /* @__PURE__ */ import_react3.default.createElement("path", { d: "M10.5 6.5v3.5H7" }), /* @__PURE__ */ import_react3.default.createElement("path", { d: "M10.2 6.8L7.2 9.8" }));
}
...
function ToolRenderChevron({ open, disabled }) {
  if (disabled === true) {
...
function InspectIcon() {
  return /* @__PURE__ */ import_react3.default.createElement(import_dsh_client_ui_primitives.IconInspectOutline12, null);
}
...
function refusalForAction(ticket, evidenceKinds, actionId) {
  const found = actionsFor(ticket, evidenceKinds).find(
    (action) => action.id === actionId
  );
  if (found === void 0) return "unknown action " + actionId;
  if (found.unavailableReason === void 0) return null;
...
function ActionBar(props) {
  ...
  const [checking, setChecking] = import_react4.default.useState(null);
  import_react4.default.useEffect(function() {
    logDebug("action bar mounted");
  }, []);
  function activate(action, opener) {
    if (action.unavailableReason !== void 0) return;
    if (props.checkAction === void 0) {
      opener();
      return;
    }
    ...
    const id = action.id;
    setChecking(id);
    void props.checkAction(id).then((refusal) => {
      if (refusal !== null) {
  ...
  return /* @__PURE__ */ import_react4.default.createElement("div", { className: "aidos-action-bar" }, buttons);
}

// src/client/inline-actions.tsx
var import_react9 = __toESM(require("react"), 1);

// src/client/allowlist-request-card.tsx
var import_react5 = __toESM(require("react"), 1);

// src/client/approval-resolution.ts
async function resolveApprovalRequest(agentId, requestId, approved, paths) {
  try {
    const clean = paths === void 0 ? void 0 : paths.map((p) => p.trim()).filter((p) => p !== "");
    const outcome = await callAidosRemote(
      "resolveApproval",
      {
        requestId,
        approved,
        ...approved && clean !== void 0 ? { paths: clean } : {}
      },
      agentId
    );
    if (approved) {
      const count = clean === void 0 ? 0 : clean.length;
...
function agentAskCount(entries) {
  return entries.filter(
    (entry) => entry.nominationId !== void 0 || entry.approvalId !== void 0
  ).length;
}
var QUEUE_TAB_ORDER = ["signoff", "approvals", "verify"];
...
var QUEUE_TAB_ICON_ACTION = {
  signoff: "signoff",
  approvals: "allowlist",
  verify: "verify"
};
function queueTabOf(entry) {
  if (entry.approvalId !== void 0) return "approvals";
  if (entry.actionId === "signoff") return "signoff";
  return "verify";
}
function groupQueueByTab(entries) {
  return QUEUE_TAB_ORDER.map((id) => ({
    id,
    label: QUEUE_TAB_LABELS[id],
    entries: entries.filter((entry) => queueTabOf(entry) === id)
  }));
}
function defaultQueueTab(entries) {
  for (const id of QUEUE_TAB_ORDER) {
    if (entries.some((entry) => queueTabOf(entry) === id)) return id;
  }
  return QUEUE_TAB_ORDER[0];
}
function queuePollMs(queueOpen) {
  return queueOpen ? 4e3 : 2e4;
}
...
function publishQueueSnapshot(snapshot) {
  currentSnapshot = snapshot;
  for (const listener of [...snapshotListeners]) listener(currentSnapshot);
}
function subscribeQueueSnapshot(listener) {
  snapshotListeners.add(listener);
  return function() {
    snapshotListeners.delete(listener);
  };
}
function selectCardApproval(snapshot, ticketId) {
...
function cardPathsAfterPoll(current, incomingPaths, dirty) {
  if (!dirty && Array.isArray(incomingPaths)) {
    return [...incomingPaths];
  }
  return [...current];
}
function dismissArmStep(armed, id) {
  if (armed !== id) return { armed: id, dismiss: false };
  return { armed: null, dismiss: true };
}
function queueButtonState(total, asks) {
...
function queueButtonTitle(state, stale) {
  const waiting = state.count === 0 ? "Nothing is waiting on you" : `${state.count} waiting on you`;
...
function AllowlistRequestCard(props) {
  const [request, setRequest] = import_react5.default.useState(null);
  const [paths, setPaths] = import_react5.default.useState([]);
  const [working, setWorking] = import_react5.default.useState(false);
  const dirtyRef = import_react5.default.useRef(false);
  const [snapshot, setSnapshot] = import_react5.default.useState(() => getQueueSnapshot());
  ...
  import_react5.default.useEffect(
    function() {
      ...
      let cancelled = false;
      void callAidosRemote("pendingApproval", { ticketId: props.ticketId }, props.agentId).then((result) => {
        if (cancelled) return;
        if (getQueueSnapshot() !== null) return;
  ...
  import_react5.default.useEffect(
    function() {
      ...
      const dirty = dirtyRef.current;
      const shared = selectCardApproval(snapshot, props.ticketId);
      const row = shared === null ? null : {
        id: shared.id,
  ...
  return /* @__PURE__ */ import_react5.default.createElement("div", { className: "aidos-approval-card" }, /* @__PURE__ */ import_react5.default.createElement("div", { className: "aidos-approval-head" }, /* @__PURE__ */ import_react5.default.createElement("span", { className: "aidos-chip aidos-chip-kind aidos-chip-approval-kind" }, /* @__PURE__ */ import_react5.default.createElement("span", { className: "aidos-chip-key" }, request.kind.toUpperCase())), /* @__PURE__ */ import_react5.default.createElement("span", { className: "aidos-approval-prompt" }, request.prompt)), /* @__PURE__ */ import_react5.default.createElement(
    "textarea",
    {
  ...
  ), createdPaths.length > 0 ? /* @__PURE__ */ import_react5.default.createElement("p", { className: "aidos-approval-created" }, createdPaths.length === 1 ? "1 path does not exist yet and will be created: " : createdPaths.length + " paths do not exist yet and will be created: ", createdPaths.join(", ")) : null, /* @__PURE__ */ import_react5.default.createElement("p", { className: "aidos-detail-note" }, "Edit the list before approving if the proposal needs amending. The agent is told the outcome either way."), /* @__PURE__ */ import_react5.default.createElement("div", { className: "aidos-form-actions" }, /* @__PURE__ */ import_react5.default.createElement("button", { className: "aidos-btn", disabled: working, onClick: () => void resolve(false) }, "Reject"), /* @__PURE__ */ import_react5.default.createElement("button", { className: "aidos-btn aidos-btn-primary", disabled: working, onClick: () => void resolve(true) }, working ? "Working\u2026" : "Approve")));
}
...
var NOT_OFFERED_IDS = /* @__PURE__ */ new Set(["builtin:retired"]);
...
function FieldRow(props) {
  return /* @__PURE__ */ import_react6.default.createElement("div", { className: "aidos-field-row" }, /* @__PURE__ */ import_react6.default.createElement("span", { className: "aidos-field-row-label" }, props.label), /* @__PURE__ */ import_react6.default.createElement("span", { className: "aidos-field-row-value" }, props.children));
}
function Collapse(props) {
  const [open, setOpen] = import_react6.default.useState(props.defaultOpen === true);
  return /* @__PURE__ */ import_react6.default.createElement(
    ...
    /* @__PURE__ */ import_react6.default.createElement("summary", null, props.summary),
    /* @__PURE__ */ import_react6.default.createElement("div", { className: "aidos-collapse-body" }, props.children)
  );
}
function ModalShell(props) {
  ...
  return /* @__PURE__ */ import_react6.default.createElement(
    ...
    /* @__PURE__ */ import_react6.default.createElement(
      ...
      /* @__PURE__ */ import_react6.default.createElement("div", { className: "aidos-modal-head" }, /* @__PURE__ */ import_react6.default.createElement("h3", { className: "aidos-modal-title" }, props.title), /* @__PURE__ */ import_react6.default.createElement(
        "button",
        {
      ...
      /* @__PURE__ */ import_react6.default.createElement("div", { className: "aidos-modal-form" }, props.children, props.onConfirm !== void 0 ? /* @__PURE__ */ import_react6.default.createElement("div", { className: "aidos-form-actions" }, /* @__PURE__ */ import_react6.default.createElement("button", { className: "aidos-btn", onClick: props.onClose, disabled: working }, "Cancel"), /* @__PURE__ */ import_react6.default.createElement(
        "button",
        {
...
function NoteField(props) {
  return /* @__PURE__ */ import_react6.default.createElement("div", { className: "aidos-modal-row" }, /* @__PURE__ */ import_react6.default.createElement("label", null, props.label), /* @__PURE__ */ import_react6.default.createElement(
    "textarea",
    {
...
function ImagePasteZone(props) {
  ...
  ), props.pasteError !== null ? /* @__PURE__ */ import_react7.default.createElement("p", { className: "aidos-evidence-paste-error" }, props.pasteError) : null);
}
...
function VerifyModal(props) {
  ...
  const [imagePath, setImagePath] = import_react7.default.useState(null);
  const [uploading, setUploading] = import_react7.default.useState(false);
  const [working, setWorking] = import_react7.default.useState(false);
  const [pasteError, setPasteError] = import_react7.default.useState(null);
  ...
  return /* @__PURE__ */ import_react7.default.createElement(AttachModal, { title: "Verify", working: working || uploading, onAttach: () => void attach(), onClose: props.onClose }, /* @__PURE__ */ import_react7.default.createElement("p", { className: "aidos-modal-body" }, "You verified this ticket hands-on. Paste (Ctrl+V) or drop a screenshot to attach it."), /* @__PURE__ */ import_react7.default.createElement(ImagePasteZone, { imagePath, uploading, pasteError, onFile: handleFile }), /* @__PURE__ */ import_react7.default.createElement(NoteField2, { note, working, onChange: setNote }));
}
function CommitPickerForm(props) {
  const [commits, setCommits] = import_react7.default.useState([]);
  const [loadError, setLoadError] = import_react7.default.useState(null);
  const [picked, setPicked] = import_react7.default.useState("");
  const [attaching, setAttaching] = import_react7.default.useState(false);
  ...
  return /* @__PURE__ */ import_react7.default.createElement("div", { className: "aidos-evidence-tailored" }, /* @__PURE__ */ import_react7.default.createElement("div", { className: "aidos-modal-row" }, /* @__PURE__ */ import_react7.default.createElement("label", null, "Recent commits"), loadError !== null ? /* @__PURE__ */ import_react7.default.createElement("p", { className: "aidos-evidence-paste-error" }, loadError) : /* @__PURE__ */ import_react7.default.createElement(
    "select",
    {
    ...
    /* @__PURE__ */ import_react7.default.createElement("option", { value: "" }, commits.length === 0 ? "Loading commits\u2026" : "Pick a commit\u2026"),
    commits.map((commit) => /* @__PURE__ */ import_react7.default.createElement("option", { value: commit.hash, key: commit.hash }, commit.hash + " " + commit.subject + " \u2014 " + commit.author))
  )), /* @__PURE__ */ import_react7.default.createElement(NoteField2, { note: props.note, working: props.working || attaching, onChange: props.setNote, label: "Note (optional)" }), /* @__PURE__ */ import_react7.default.createElement("div", { className: "aidos-form-actions" }, /* @__PURE__ */ import_react7.default.createElement(
    "button",
    {
      className: "aidos-btn aidos-btn-primary",
...
function EvalCriteriaForm(props) {
  const [criteriaText, setCriteriaText] = import_react7.default.useState("");
  const [note, setNote] = import_react7.default.useState("");
  const [working, setWorking] = import_react7.default.useState(false);
  ...
  return /* @__PURE__ */ import_react7.default.createElement("div", { className: "aidos-evidence-tailored" }, /* @__PURE__ */ import_react7.default.createElement(
    LinesField,
    {
  ...
  ), /* @__PURE__ */ import_react7.default.createElement(NoteField2, { note, working, onChange: setNote }), /* @__PURE__ */ import_react7.default.createElement("div", { className: "aidos-form-actions" }, /* @__PURE__ */ import_react7.default.createElement(
    "button",
    {
      className: "aidos-btn aidos-btn-primary",
...
function AgentReportForm(props) {
  const [reportText, setReportText] = import_react7.default.useState("");
  const [note, setNote] = import_react7.default.useState("");
  const [working, setWorking] = import_react7.default.useState(false);
  async function attach() {
  ...
  return /* @__PURE__ */ import_react7.default.createElement("div", { className: "aidos-evidence-tailored" }, /* @__PURE__ */ import_react7.default.createElement("div", { className: "aidos-modal-row" }, /* @__PURE__ */ import_react7.default.createElement("label", null, "Report"), /* @__PURE__ */ import_react7.default.createElement(
    "textarea",
    {
...
function CheckResultForm(props) {
  const [command, setCommand] = import_react7.default.useState("");
  const [result, setResult] = import_react7.default.useState("");
  const [note, setNote] = import_react7.default.useState("");
  const [working, setWorking] = import_react7.default.useState(false);
  async function attach() {
  ...
  return /* @__PURE__ */ import_react7.default.createElement("div", { className: "aidos-evidence-tailored" }, /* @__PURE__ */ import_react7.default.createElement("div", { className: "aidos-modal-row" }, /* @__PURE__ */ import_react7.default.createElement("label", null, "Command"), /* @__PURE__ */ import_react7.default.createElement(
    "input",
    {
      type: "text",
  ...
  )), /* @__PURE__ */ import_react7.default.createElement("div", { className: "aidos-modal-row" }, /* @__PURE__ */ import_react7.default.createElement("label", null, "Result"), /* @__PURE__ */ import_react7.default.createElement(
    "select",
    {
      className: "aidos-evidence-attach-kind-select",
      value: result,
    ...
    /* @__PURE__ */ import_react7.default.createElement("option", { value: "" }, "Choose a result\u2026"),
    /* @__PURE__ */ import_react7.default.createElement("option", { value: "pass" }, "Pass"),
    /* @__PURE__ */ import_react7.default.createElement("option", { value: "fail" }, "Fail")
  )), /* @__PURE__ */ import_react7.default.createElement(NoteField2, { note, working, onChange: setNote }), /* @__PURE__ */ import_react7.default.createElement("div", { className: "aidos-form-actions" }, /* @__PURE__ */ import_react7.default.createElement(
    "button",
    {
      className: "aidos-btn aidos-btn-primary",
...
function AfterShotForm(props) {
  ...
  return /* @__PURE__ */ import_react7.default.createElement("div", { className: "aidos-evidence-tailored" }, /* @__PURE__ */ import_react7.default.createElement(ImagePasteZone, { imagePath, uploading, pasteError, onFile: handleFile }), /* @__PURE__ */ import_react7.default.createElement(NoteField2, { note, working, onChange: setNote }), /* @__PURE__ */ import_react7.default.createElement("div", { className: "aidos-form-actions" }, /* @__PURE__ */ import_react7.default.createElement(
    "button",
    {
      className: "aidos-btn aidos-btn-primary",
...
function TailoredForm(props) {
  const [note, setNote] = import_react7.default.useState("");
  const [pathsText, setPathsText] = import_react7.default.useState("");
  const [payloadText, setPayloadText] = import_react7.default.useState("");
  const [working, setWorking] = import_react7.default.useState(false);
  ...
  async function grantAllowlist(paths) {
    if (working) return;
    setWorking(true);
    try {
      await callAidosRemote(
  ...
  if (props.kind === "builtin:eval_criteria") {
    return /* @__PURE__ */ import_react7.default.createElement(EvalCriteriaForm, { ticketId: props.ticketId, agentId: props.agentId, onAttached: props.onAttached });
  }
  if (props.kind === "builtin:agent_report") {
    return /* @__PURE__ */ import_react7.default.createElement(AgentReportForm, { ticketId: props.ticketId, agentId: props.agentId, onAttached: props.onAttached });
  }
  ...
  if (props.kind === "builtin:after_shot") {
    return /* @__PURE__ */ import_react7.default.createElement(AfterShotForm, { ticketId: props.ticketId, agentId: props.agentId, onAttached: props.onAttached });
  }
  if (props.kind === "builtin:user_commit") {
    return /* @__PURE__ */ import_react7.default.createElement(CommitPickerForm, { ticketId: props.ticketId, agentId: props.agentId, onAttached: props.onAttached, note, setNote, working });
  }
  ...
  if (verdictLabel !== void 0) {
    return /* @__PURE__ */ import_react7.default.createElement("div", { className: "aidos-evidence-tailored" }, /* @__PURE__ */ import_react7.default.createElement(NoteField2, { note, working, onChange: setNote, label: verdictLabel }), /* @__PURE__ */ import_react7.default.createElement("div", { className: "aidos-form-actions" }, /* @__PURE__ */ import_react7.default.createElement(
      "button",
      {
        className: "aidos-btn aidos-btn-primary",
  ...
  return /* @__PURE__ */ import_react7.default.createElement("div", { className: "aidos-evidence-tailored" }, /* @__PURE__ */ import_react7.default.createElement(Collapse, { summary: "Raw JSON (optional object)", defaultOpen: false }, /* @__PURE__ */ import_react7.default.createElement("div", { className: "aidos-modal-row" }, /* @__PURE__ */ import_react7.default.createElement(
    "textarea",
    {
      className: "aidos-evidence-attach-note",
      value: payloadText,
  ...
  )), parseError !== null ? /* @__PURE__ */ import_react7.default.createElement("p", { className: "aidos-evidence-paste-error" }, parseError) : null), /* @__PURE__ */ import_react7.default.createElement(NoteField2, { note, working, onChange: setNote }), /* @__PURE__ */ import_react7.default.createElement("div", { className: "aidos-form-actions" }, /* @__PURE__ */ import_react7.default.createElement(
    "button",
    {
      className: "aidos-btn aidos-btn-primary",
...
function EvidenceAttach(props) {
  ...
  const [kind, setKind] = import_react7.default.useState(kinds.length > 0 ? kinds[0].id : "");
  const [lastRow, setLastRow] = import_react7.default.useState(null);
  const remainingKinds = kinds.filter(
    (k2) => k2.id !== "builtin:user_signoff" && k2.id !== "builtin:user_verified"
  );
  void lastRow;
  return /* @__PURE__ */ import_react7.default.createElement("div", { className: "aidos-evidence-attach" }, /* @__PURE__ */ import_react7.default.createElement("div", { className: "aidos-modal-row" }, /* @__PURE__ */ import_react7.default.createElement("label", null, "Other evidence kinds"), /* @__PURE__ */ import_react7.default.createElement(
    "select",
    {
      className: "aidos-evidence-attach-kind-select",
      value: kind,
    ...
    remainingKinds.map((descriptor) => /* @__PURE__ */ import_react7.default.createElement("option", { value: descriptor.id, key: descriptor.id }, descriptor.label))
  )), /* @__PURE__ */ import_react7.default.createElement(TailoredForm, { ticketId: props.ticketId, agentId: props.agentId, kind, onAttached: () => props.onAttached?.() }));
}

// src/client/signoff-dialog.tsx
var import_react8 = __toESM(require("react"), 1);
function SignoffDialog(props) {
  const [working, setWorking] = import_react8.default.useState(false);
  const [note, setNote] = import_react8.default.useState("");
  const [pathsText, setPathsText] = import_react8.default.useState(
    ...
    try {
      ...
      if (paths.length > 0) {
        try {
          ...
          grantFailure = error instanceof Error ? error.message : String(error);
        }
      }
      await callAidosRemote(
        "userMoveTicket",
      ...
      if (grantFailure === null) {
        showToast(
          paths.length > 0 ? "Signed off \u2014 " + paths.length + " path(s) granted" : "Signed off",
          "success"
        );
      } else {
        showToast(
          "Signed off and moved, but the allowlist was refused: " + grantFailure + " \u2014 set the paths from the ticket's allowlist editor",
          "refusal"
        );
      }
      props.onClose();
  ...
  return /* @__PURE__ */ import_react8.default.createElement(
    ...
    /* @__PURE__ */ import_react8.default.createElement("p", { className: "aidos-modal-body" }, "Signoff moves this to in progress and grants the agent write access inside the allowlist below. Signoff alone grants access to nothing, so name the files here \u2014 or leave it empty and scope them later."),
...
function isInlineActionId(value) {
  return value === "signoff" || value === "verify" || value === "mark-done";
}
function refusalForInlineAction(ticket, evidenceKinds, actionId) {
...
async function checkInlineActionAvailable(sessionId, boardKey, actionId) {
  if (actionId === "mark-done") return null;
  ...
  if (row === null) {
    return "#" + boardKey + " is not on this board (it may belong to another session)";
  }
  const kinds = (evidence[boardKey] ?? []).map((entry) => entry.kind).filter((kind) => typeof kind === "string");
  return refusalForInlineAction(row, kinds, actionId);
}
async function checkBoardActionAvailable(sessionId, boardKey, actionId) {
  const result = await callAidosRemote("workspaceTickets", {}, sessionId);
  const tickets = Array.isArray(result) ? result : result.tickets ?? [];
  const evidence = Array.isArray(result) ? {} : result.evidence ?? {};
  const row = tickets.find((ticket) => boardKeyOf(ticket) === boardKey) ?? null;
  ...
  const kinds = [...evidence[boardKey] ?? [], ...evidence[canonical] ?? []].map((entry) => entry.kind).filter((kind) => typeof kind === "string");
  return refusalForAction(row, kinds, actionId);
}
function InlineTicketAction(props) {
  const [open, setOpen] = import_react9.default.useState(null);
  const [checking, setChecking] = import_react9.default.useState(false);
  function activate() {
    ...
    setChecking(true);
    void checkInlineActionAvailable(props.sessionId, props.boardKey, action).then((refusal) => {
      if (refusal !== null) {
        showToast(refusal, "refusal");
        return;
      }
  ...
  const title = props.title ?? ticketTitle(props.sessionId, props.boardKey) ?? props.boardKey;
  ...
  ), open === "signoff" ? /* @__PURE__ */ import_react9.default.createElement(
    SignoffDialog,
    {
      open: true,
  ...
  ) : null, open === "verify" ? /* @__PURE__ */ import_react9.default.createElement(
    VerifyModal,
    {
      ticketId: props.boardKey,
      agentId: props.sessionId,
...
function InlineAllowlistApproval(props) {
  return /* @__PURE__ */ import_react9.default.createElement("div", { className: "aidos-inline-approval" }, /* @__PURE__ */ import_react9.default.createElement(AllowlistRequestCard, { ticketId: props.boardKey, agentId: props.sessionId }));
}
...
function AllowlistEditor(props) {
  ...
  const [others, setOthers] = import_react10.default.useState([]);
  const [saving, setSaving] = import_react10.default.useState(false);
  ...
  return /* @__PURE__ */ import_react10.default.createElement(
    ...
    /* @__PURE__ */ import_react10.default.createElement(
      ...
      /* @__PURE__ */ import_react10.default.createElement("div", { className: "aidos-modal-head" }, /* @__PURE__ */ import_react10.default.createElement("h3", { className: "aidos-modal-title" }, "File allowlist"), /* @__PURE__ */ import_react10.default.createElement(
        "button",
        {
          className: "aidos-close-btn",
          onClick: () => {
      ...
      /* @__PURE__ */ import_react10.default.createElement("div", { className: "aidos-modal-form" }, /* @__PURE__ */ import_react10.default.createElement("div", { className: "aidos-modal-row" }, /* @__PURE__ */ import_react10.default.createElement("label", null, "One path per line. Saving grants these paths on top of the current list \u2014 a write outside the granted set refuses while the ticket is in progress. To revoke a path, delete its grant row from the ticket's evidence."), /* @__PURE__ */ import_react10.default.createElement(
        "textarea",
        {
          className: "aidos-allowlist-input",
          value: text,
      ...
      )), others.length > 0 ? /* @__PURE__ */ import_react10.default.createElement("div", { className: "aidos-modal-row aidos-allowlist-preview" }, /* @__PURE__ */ import_react10.default.createElement("label", null, "Also allowed by other in-progress tickets"), /* @__PURE__ */ import_react10.default.createElement("ul", null, others.map((path) => /* @__PURE__ */ import_react10.default.createElement("li", { key: path }, path)))) : null, /* @__PURE__ */ import_react10.default.createElement(
        "button",
        {
          className: "aidos-btn aidos-btn-primary",
          disabled: saving,
...
function NoteText(props) {
  return /* @__PURE__ */ import_react11.default.createElement("span", { className: "aidos-evidence-note-text" }, props.text);
}
function RawJsonDisclosure(props) {
  ...
  return /* @__PURE__ */ import_react11.default.createElement(
    ...
    /* @__PURE__ */ import_react11.default.createElement("summary", null, "raw payload"),
    /* @__PURE__ */ import_react11.default.createElement("pre", { className: "aidos-evidence-payload-json" }, JSON.stringify(props.payload, null, 2))
  );
}
...
function EvidencePayloadView(props) {
  ...
  if (kind === "builtin:file_allowlist" && Array.isArray(payload.paths)) {
    return /* @__PURE__ */ import_react11.default.createElement("div", { className: "aidos-evidence-fields" }, /* @__PURE__ */ import_react11.default.createElement(FieldRow, { label: "Paths" }, /* @__PURE__ */ import_react11.default.createElement("ul", { className: "aidos-evidence-payload-list" }, payload.paths.map((path) => /* @__PURE__ */ import_react11.default.createElement("li", { key: path }, path)))), note !== null ? /* @__PURE__ */ import_react11.default.createElement(FieldRow, { label: "Note" }, /* @__PURE__ */ import_react11.default.createElement(NoteText, { text: note })) : null, /* @__PURE__ */ import_react11.default.createElement(RawJsonDisclosure, { payload }));
  }
  if (kind === "builtin:imported_state" && typeof payload.claimed_state === "string") {
    return /* @__PURE__ */ import_react11.default.createElement("div", { className: "aidos-evidence-fields" }, /* @__PURE__ */ import_react11.default.createElement(FieldRow, { label: "Claimed state" }, /* @__PURE__ */ import_react11.default.createElement("span", { className: "aidos-evidence-note-text" }, payload.claimed_state)), typeof payload.source === "string" ? /* @__PURE__ */ import_react11.default.createElement(FieldRow, { label: "Source" }, payload.source) : null, note !== null ? /* @__PURE__ */ import_react11.default.createElement(FieldRow, { label: "Note" }, /* @__PURE__ */ import_react11.default.createElement(NoteText, { text: note })) : null, /* @__PURE__ */ import_react11.default.createElement(RawJsonDisclosure, { payload }));
  }
  if (typeof payload.imagePath === "string") {
    return /* @__PURE__ */ import_react11.default.createElement("div", { className: "aidos-evidence-fields" }, /* @__PURE__ */ import_react11.default.createElement(FieldRow, { label: "Screenshot" }, /* @__PURE__ */ import_react11.default.createElement("img", { className: "aidos-evidence-image", src: payload.imagePath, alt: note ?? "evidence screenshot" }), /* @__PURE__ */ import_react11.default.createElement("span", { className: "aidos-evidence-image-path" }, payload.imagePath)), note !== null ? /* @__PURE__ */ import_react11.default.createElement(FieldRow, { label: "Note" }, /* @__PURE__ */ import_react11.default.createElement(NoteText, { text: note })) : null, /* @__PURE__ */ import_react11.default.createElement(RawJsonDisclosure, { payload }));
  }
  if (typeof payload.commit === "string") {
    return /* @__PURE__ */ import_react11.default.createElement("div", { className: "aidos-evidence-fields" }, /* @__PURE__ */ import_react11.default.createElement(FieldRow, { label: "Commit" }, /* @__PURE__ */ import_react11.default.createElement("code", null, String(payload.commit).slice(0, 12)), typeof payload.subject === "string" ? /* @__PURE__ */ import_react11.default.createElement(NoteText, { text: " " + payload.subject }) : null), typeof payload.author === "string" ? /* @__PURE__ */ import_react11.default.createElement(FieldRow, { label: "Committed by" }, payload.author) : null, typeof payload.branch === "string" ? /* @__PURE__ */ import_react11.default.createElement(FieldRow, { label: "Branch" }, payload.branch) : null, note !== null ? /* @__PURE__ */ import_react11.default.createElement(FieldRow, { label: "Note" }, /* @__PURE__ */ import_react11.default.createElement(NoteText, { text: note })) : null, /* @__PURE__ */ import_react11.default.createElement(RawJsonDisclosure, { payload }));
  }
  ...
  return /* @__PURE__ */ import_react11.default.createElement("div", { className: "aidos-evidence-fields" }, textFields.map(([key, value]) => /* @__PURE__ */ import_react11.default.createElement(FieldRow, { key, label: key }, /* @__PURE__ */ import_react11.default.createElement(NoteText, { text: value }))), otherFields.map(([key, value]) => /* @__PURE__ */ import_react11.default.createElement(FieldRow, { key, label: key }, /* @__PURE__ */ import_react11.default.createElement("code", null, isImage(String(value)) ? String(value) : JSON.stringify(value)))), note !== null ? /* @__PURE__ */ import_react11.default.createElement(FieldRow, { label: "Note" }, /* @__PURE__ */ import_react11.default.createElement(NoteText, { text: note })) : null, /* @__PURE__ */ import_react11.default.createElement(RawJsonDisclosure, { payload }));
}
...
function EvidenceViewer(props) {
  ...
  return /* @__PURE__ */ import_react12.default.createElement(
    ...
    /* @__PURE__ */ import_react12.default.createElement(
      ...
      /* @__PURE__ */ import_react12.default.createElement("div", { className: "aidos-modal-head" }, /* @__PURE__ */ import_react12.default.createElement("h3", { className: "aidos-modal-title" }, /* @__PURE__ */ import_react12.default.createElement(
        "span",
        {
          className: "aidos-chip aidos-chip-kind",
          style: { ["--chip-hue"]: kindColor(row.kind) }
        },
        /* @__PURE__ */ import_react12.default.createElement("span", { className: "aidos-chip-key" }, kindKeyword(row.kind))
      ), " " + row.kind), /* @__PURE__ */ import_react12.default.createElement(
        "button",
        {
          className: "aidos-close-btn",
          onClick: props.onClose,
      ...
      /* @__PURE__ */ import_react12.default.createElement("div", { className: "aidos-modal-form" }, /* @__PURE__ */ import_react12.default.createElement("div", { className: "aidos-evidence-fields" }, /* @__PURE__ */ import_react12.default.createElement(FieldRow, { label: "Author" }, row.author), /* @__PURE__ */ import_react12.default.createElement(FieldRow, { label: "At" }, typeof row.at === "number" ? new Date(row.at * 1e3).toISOString() : "unknown")), /* @__PURE__ */ import_react12.default.createElement(EvidencePayloadView, { row }))
    )
  );
}
...
function EvidenceStrip(props) {
  ...
  return /* @__PURE__ */ import_react13.default.createElement("li", { className: "aidos-evidence-strip" }, /* @__PURE__ */ import_react13.default.createElement("div", { className: "aidos-evidence-strip-main" }, /* @__PURE__ */ import_react13.default.createElement(
    "span",
    {
      className: "aidos-chip aidos-chip-kind",
      style: { ["--chip-hue"]: kindColor(row.kind) },
  ...
  ), props.standing === "verified" || props.standing === "invalidated" ? /* @__PURE__ */ import_react13.default.createElement(
    "span",
    {
      className: "aidos-review-standing aidos-review-standing-" + props.standing,
      title: props.standingReason ?? "",
      "data-dsh-tip": "",
      "aria-label": props.standing === "verified" ? "This review ran through the configured reviewer chain" : "This review did not run on the chain it declared"
    },
    props.standing === "verified" ? "\u2713" : "\u26A0"
  ) : null, /* @__PURE__ */ import_react13.default.createElement("span", { className: "aidos-evidence-strip-body" }, excerpt !== null ? /* @__PURE__ */ import_react13.default.createElement("span", { className: "aidos-evidence-strip-excerpt" }, excerpt) : /* @__PURE__ */ import_react13.default.createElement("span", { className: "aidos-evidence-strip-kind-name" }, row.kind), /* @__PURE__ */ import_react13.default.createElement("span", { className: "aidos-evidence-strip-meta" }, row.author, when !== null ? " \xB7 " + when : "", props.criterionLabel !== void 0 ? " \xB7 criterion: " + props.criterionLabel : null)), /* @__PURE__ */ import_react13.default.createElement("span", { className: "aidos-evidence-strip-actions" }, props.onView !== void 0 ? /* @__PURE__ */ import_react13.default.createElement(
    "button",
    {
    ...
    /* @__PURE__ */ import_react13.default.createElement(PopOutIcon, null)
  ) : null, props.onUnlink !== void 0 ? /* @__PURE__ */ import_react13.default.createElement(
    "button",
    {
    ...
    "\u2A02"
  ) : null, props.onDelete !== void 0 ? /* @__PURE__ */ import_react13.default.createElement(
    "button",
    {
...
function CriterionLinker(props) {
  const [busyAt, setBusyAt] = import_react14.default.useState(null);
  const [viewed, setViewed] = import_react14.default.useState(null);
  ...
  return /* @__PURE__ */ import_react14.default.createElement("div", { className: "aidos-criterion-blocks" }, viewed === null ? null : /* @__PURE__ */ import_react14.default.createElement(
    EvidenceViewer,
    {
      row: viewed,
      onClose: () => {
    ...
    return /* @__PURE__ */ import_react14.default.createElement("div", { className: "aidos-criterion-block", key: label }, /* @__PURE__ */ import_react14.default.createElement("div", { className: "aidos-criterion-label" }, label), linked.length > 0 ? /* @__PURE__ */ import_react14.default.createElement("ul", { className: "aidos-criterion-evidence" }, linked.map((row) => /* @__PURE__ */ import_react14.default.createElement(
      EvidenceStrip,
      {
    ...
    ))) : /* @__PURE__ */ import_react14.default.createElement("p", { className: "aidos-detail-note" }, "No evidence linked."), !props.readOnly && options.length > 0 ? /* @__PURE__ */ import_react14.default.createElement("div", { className: "aidos-criterion-linker" }, /* @__PURE__ */ import_react14.default.createElement(
      "select",
      {
        value,
        onChange: (event) => {
      ...
      /* @__PURE__ */ import_react14.default.createElement("option", { value: "" }, "Link an evidence row\u2026"),
      options.map((row) => /* @__PURE__ */ import_react14.default.createElement("option", { key: String(row.at) + ":" + row.kind, value: String(row.at) + ":" + row.kind }, evidenceOptionLabel(row)))
...
// src/client/strip-harness-chrome.ts
var REF_CHIP_OPEN_ATTRS = "[^>]*?(?:refChip|ref-chip|data-ref-chip)[^>]*?";
var REF_CHIP_SPAN = new RegExp(
  "<span\\b" + REF_CHIP_OPEN_ATTRS + ">([\\s\\S]*?)</span>",
  "g"
);
var REF_CHIP_SPAN_EMPTY = new RegExp(
  "<span\\b" + REF_CHIP_OPEN_ATTRS + "/>",
  "g"
);
var RESIDUAL_CHROME = /<[A-Za-z][^<>]*ref[_-]?chip/i;
function stripHarnessChrome(text) {
  const stripped = text.replace(REF_CHIP_SPAN, (_match, label) => label).replace(REF_CHIP_SPAN_EMPTY, "");
  const residual = RESIDUAL_CHROME.exec(stripped);
  if (residual !== null) {
    const at2 = Math.max(0, residual.index - 40);
    throw new Error(
      "#182: unrecognised harness ref-chip variant in text headed for an aidos renderer -- refusing to pass it through (a silent pass re-creates the escaped-markup bug per variant). Teach stripHarnessChrome the new shape. Near: " + JSON.stringify(stripped.slice(at2, residual.index + 120))
    );
  }
  return stripped;
}

// src/client/safe-markdown.ts
var HTML_ESCAPES = {
  "&": "&amp;",
  "<": "&lt;",
  ">": "&gt;",
...
function isSafeUrl(url) {
  const normalized = url.replace(
    /&#(x?)([0-9a-f]+);?/gi,
    (_match, hex, code) => String.fromCharCode(parseInt(code, hex === "" ? 10 : 16))
...
function neutralizeUrls(html) {
  return html.replace(
    /(\s(?:href|src)=")([^"]*)(")/gi,
    (match, prefix, url, suffix) => isSafeUrl(url) ? match : prefix + "#" + suffix
  );
}
function renderMarkdownSafe(text) {
  if (text === "") return "";
  const parsed = f.parse(escapeHtml(stripHarnessChrome(text)), { async: false });
...
function FieldEditor(props) {
  const [editing, setEditing] = import_react15.default.useState(false);
  const [draft, setDraft] = import_react15.default.useState(String(props.value));
  ...
  if (editing) {
    ...
    return /* @__PURE__ */ import_react15.default.createElement("div", { className: "aidos-field-editor" }, isTextarea ? /* @__PURE__ */ import_react15.default.createElement(
      "textarea",
      {
    ...
    ), /* @__PURE__ */ import_react15.default.createElement("span", null, /* @__PURE__ */ import_react15.default.createElement("button", { className: "aidos-btn", disabled: saving, onClick: save }, "Save"), " ", /* @__PURE__ */ import_react15.default.createElement("button", { className: "aidos-btn", disabled: saving, onClick: cancel }, "Cancel")));
  }
  return /* @__PURE__ */ import_react15.default.createElement("div", { className: "aidos-field-editor" }, /* @__PURE__ */ import_react15.default.createElement("span", null, props.children !== void 0 ? props.children : String(props.value), " ", /* @__PURE__ */ import_react15.default.createElement(
    "button",
    {
      className: "aidos-icon-btn",
      title: "Edit",
    ...
    /* @__PURE__ */ import_react15.default.createElement(PencilIcon, null)
  )));
}
...
function CommentsSection(props) {
  ...
  const [draft, setDraft] = import_react16.default.useState("");
  const [sending, setSending] = import_react16.default.useState(false);
  ...
  const rows = newestFirst.map((comment, index) => {
    ...
    return /* @__PURE__ */ import_react16.default.createElement("div", { className: "aidos-comment", key: index }, /* @__PURE__ */ import_react16.default.createElement("div", null, /* @__PURE__ */ import_react16.default.createElement("span", { className: "aidos-evidence-author" }, comment.author)), /* @__PURE__ */ import_react16.default.createElement("p", { className: "aidos-detail-body" }, comment.text), /* @__PURE__ */ import_react16.default.createElement("p", { className: "aidos-detail-note" }, time));
  });
  return /* @__PURE__ */ import_react16.default.createElement("details", { className: "aidos-panel", open: comments.length !== 1 }, /* @__PURE__ */ import_react16.default.createElement("summary", { className: "aidos-panel-head" }, /* @__PURE__ */ import_react16.default.createElement("h4", { className: "aidos-panel-title" }, "Comments")), /* @__PURE__ */ import_react16.default.createElement("div", { className: "aidos-panel-body" }, rows.length === 0 ? /* @__PURE__ */ import_react16.default.createElement("p", { className: "aidos-detail-note" }, "No comments yet.") : rows, /* @__PURE__ */ import_react16.default.createElement(
    "textarea",
    {
  ...
  ), /* @__PURE__ */ import_react16.default.createElement("div", { className: "aidos-form-actions" }, /* @__PURE__ */ import_react16.default.createElement(
    "button",
    {
...
var STRIP_CACHE_LIMIT = 1024;
var stripMarkupCache = /* @__PURE__ */ new Map();
function TicketStrip(props) {
  const key = String(props.ticket.id);
  const hit = stripMarkupCache.get(key);
  if (hit !== void 0 && stripPropsEqual(hit.props, props)) {
    return hit.element;
  }
...
function stripPropsEqual(a, b2) {
  const ta = a.ticket;
  const tb = b2.ticket;
  return ta.id === tb.id && ta.title === tb.title && ta.state === tb.state && ta.slug === tb.slug && ta.workspaceKey === tb.workspaceKey && (ta.gatePresent ?? null) === (tb.gatePresent ?? null) && (ta.gateTotal ?? null) === (tb.gateTotal ?? null) && (ta.criteria ?? null) === (tb.criteria ?? null) && (a.showState ?? null) === (b2.showState ?? null) && a.meta === b2.meta && a.actions === b2.actions && a.actionIcon === b2.actionIcon && (a.actionHint ?? null) === (b2.actionHint ?? null) && (a.expanded ?? false) === (b2.expanded ?? false) && (a.working ?? false) === (b2.working ?? false) && (a.highlighted ?? false) === (b2.highlighted ?? false) && (a.awaitingApproval ?? false) === (b2.awaitingApproval ?? false);
}
function renderTicketStrip(props) {
  ...
  return /* @__PURE__ */ import_react17.default.createElement("li", { className }, /* @__PURE__ */ import_react17.default.createElement("div", { className: "aidos-ticket-strip-main" }, /* @__PURE__ */ import_react17.default.createElement("span", { className: "aidos-ticket-strip-idcol" }, /* @__PURE__ */ import_react17.default.createElement(
    ...
    {
      className: "aidos-chip aidos-chip-id",
      style: { ["--chip-hue"]: idColor(full) },
      title: full,
      "data-dsh-tip": ""
    },
    ticketChipLabel(ticket)
  ), props.showState === false ? null : /* @__PURE__ */ import_react17.default.createElement(
    "span",
    {
    ...
    (() => {
      ...
      return /* @__PURE__ */ import_react17.default.createElement(
        "span",
        {
          className: "aidos-chip aidos-chip-metric aidos-chip-gate" + (failed ? " aidos-chip-fail" : ""),
          "aria-label": sentence,
          title: sentence,
          "data-dsh-tip": ""
        },
...
function SendBackModal(props) {
  const [reason, setReason] = import_react18.default.useState("");
  ...
  return /* @__PURE__ */ import_react18.default.createElement(
    ...
    /* @__PURE__ */ import_react18.default.createElement("p", { className: "aidos-modal-body" }, "Send the ticket back to in progress. The reason attaches as a comment."),
    /* @__PURE__ */ import_react18.default.createElement(NoteField, { label: "Reason", value: reason, working, onChange: setReason })
  );
}
...
function MarkDoneModal(props) {
  const [step, setStep] = import_react19.default.useState(1);
  const [finalComment, setFinalComment] = import_react19.default.useState("");
  const [working, setWorking] = import_react19.default.useState(false);
  const [viewed, setViewed] = import_react19.default.useState(null);
  const [verifiedNow, setVerifiedNow] = import_react19.default.useState(false);
  const [promptToVerify, setPromptToVerify] = import_react19.default.useState(null);
  ...
  if (gated && promptToVerify === null) {
    return /* @__PURE__ */ import_react19.default.createElement(ModalShell, { title: "Mark done", working, onClose: props.onClose }, /* @__PURE__ */ import_react19.default.createElement("div", { className: "aidos-modal-form" }, /* @__PURE__ */ import_react19.default.createElement("p", { className: "aidos-modal-body" }, "This ticket has no verification row yet. Marking it done needs your hands-on check first \u2014 verify it, and the row you attach is what this close stands on."), /* @__PURE__ */ import_react19.default.createElement("div", { className: "aidos-form-actions" }, /* @__PURE__ */ import_react19.default.createElement(
      "button",
      {
        className: "aidos-btn aidos-btn-primary",
        disabled: working,
  ...
  if (gated && promptToVerify === "force") {
    return /* @__PURE__ */ import_react19.default.createElement(ModalShell, { title: "Mark done", working, onClose: props.onClose }, /* @__PURE__ */ import_react19.default.createElement("div", { className: "aidos-modal-form" }, /* @__PURE__ */ import_react19.default.createElement("p", { className: "aidos-modal-body" }, "Forcing closes this ticket with NO verification row behind it. The board will show it done with nothing verified \u2014 choose this only when the check genuinely does not apply."), /* @__PURE__ */ import_react19.default.createElement(
      NoteField,
      {
    ...
    ), /* @__PURE__ */ import_react19.default.createElement("div", { className: "aidos-form-actions" }, /* @__PURE__ */ import_react19.default.createElement(
      "button",
      {
        className: "aidos-btn",
        disabled: working,
  ...
  return /* @__PURE__ */ import_react19.default.createElement(ModalShell, { title: "Mark done", working, onClose: props.onClose }, viewed === null ? null : /* @__PURE__ */ import_react19.default.createElement(
    EvidenceViewer,
    {
      row: viewed,
      onClose: () => {
  ...
  ), step === 1 ? /* @__PURE__ */ import_react19.default.createElement("div", { className: "aidos-modal-form" }, /* @__PURE__ */ import_react19.default.createElement("p", { className: "aidos-modal-body" }, "The ticket criteria, with their evidence:"), criteriaLines2.length === 0 ? /* @__PURE__ */ import_react19.default.createElement("p", { className: "aidos-detail-note" }, "No criteria on this ticket.") : /* @__PURE__ */ import_react19.default.createElement(
    CriterionLinker,
    {
    ...
    "Continue"
  ))) : /* @__PURE__ */ import_react19.default.createElement("div", { className: "aidos-modal-form" }, /* @__PURE__ */ import_react19.default.createElement("p", { className: "aidos-modal-body" }, "The evidence on this ticket:"), props.evidence.length === 0 ? /* @__PURE__ */ import_react19.default.createElement("p", { className: "aidos-detail-note" }, "No evidence rows yet.") : /* @__PURE__ */ import_react19.default.createElement("ul", { className: "aidos-evidence-list" }, props.evidence.map((row, index) => /* @__PURE__ */ import_react19.default.createElement(
    EvidenceStrip,
    {
...
// src/client/retired-panel.tsx
var import_react20 = __toESM(require("react"), 1);
function retiredWhen(at2) {
  return new Date(at2 * 1e3).toLocaleString();
}
function RetiredPanel(props) {
  const [rows, setRows] = import_react20.default.useState(null);
  const [error, setError] = import_react20.default.useState(null);
  const [workingKey, setWorkingKey] = import_react20.default.useState(null);
  const [expandedKey, setExpandedKey] = import_react20.default.useState(null);
  const refresh = import_react20.default.useCallback(function() {
    setError(null);
    callAidosRemote("retiredTickets", {}, props.sessionId).then((result) => {
  ...
  if (error !== null) {
    return /* @__PURE__ */ import_react20.default.createElement("div", { className: "aidos-retired-panel" }, /* @__PURE__ */ import_react20.default.createElement("p", { className: "aidos-retired-error" }, "The retired list could not be read: ", error), /* @__PURE__ */ import_react20.default.createElement("button", { className: "aidos-btn", onClick: refresh }, "Retry"));
  }
  if (rows === null) {
    return /* @__PURE__ */ import_react20.default.createElement("div", { className: "aidos-retired-panel", role: "status" }, /* @__PURE__ */ import_react20.default.createElement("span", { className: "aidos-merge-spinner", "aria-hidden": "true" }), /* @__PURE__ */ import_react20.default.createElement("span", { className: "aidos-retired-empty" }, "Loading retired tickets\u2026"));
  }
  if (rows.length === 0) {
    return /* @__PURE__ */ import_react20.default.createElement("div", { className: "aidos-retired-panel" }, /* @__PURE__ */ import_react20.default.createElement("p", { className: "aidos-retired-empty" }, "Nothing is retired. Retired tickets are hidden from the board, the queue and the agent's reads \u2014 they appear here until un-retired."));
  }
  return /* @__PURE__ */ import_react20.default.createElement("ul", { className: "aidos-retired-panel" }, rows.map((row) => {
    const key = boardKeyOf(row);
    ...
    return /* @__PURE__ */ import_react20.default.createElement(
      TicketStrip,
      {
        ...
        actionIcon: /* @__PURE__ */ import_react20.default.createElement(UnretireIcon, null),
        actionHint: "Un-retire this ticket",
        actions: /* @__PURE__ */ import_react20.default.createElement(
          "button",
          {
            className: "aidos-btn aidos-btn-primary",
        ...
        onOpen: props.onOpen === void 0 ? void 0 : () => props.onOpen?.(key),
        meta: /* @__PURE__ */ import_react20.default.createElement(import_react20.default.Fragment, null, /* @__PURE__ */ import_react20.default.createElement("span", { title: reason, "data-dsh-tip": "" }, reason), /* @__PURE__ */ import_react20.default.createElement("span", null, " \u2014 by ", retirement.author, " \xB7 ", retiredWhen(retirement.at)), directTargets.length > 0 ? /* @__PURE__ */ import_react20.default.createElement("span", { className: "aidos-retired-supersede" }, directTargets.map(
          (target) => target.known ? `#${target.id} ${target.title}` : target.ref
        ).join(", ")) : null, onward.length > 0 ? /* @__PURE__ */ import_react20.default.createElement("span", { className: "aidos-retired-supersede" }, "chain ends at ", onward.join(", ")) : null, retirement.chainCycle ? /* @__PURE__ */ import_react20.default.createElement("span", { className: "aidos-retired-supersede" }, "supersede cycle cut here") : null)
      }
    );
  }));
}
function RetireDialog(props) {
  ...
  const [reason, setReason] = import_react20.default.useState("");
  const [supersedesText, setSupersedesText] = import_react20.default.useState("");
  import_react20.default.useEffect(function() {
...
async function submitTicketForReview(agentId, ticketIdKey) {
  await callAidosRemote(
    "userMoveTicket",
...
function DescriptionPanel(props) {
  ...
  const html = empty ? "" : renderMarkdownSafe(text);
  async function save() {
    if (saving) return;
    setSaving(true);
    try {
...
function EvidencePanel(props) {
  const [standings, setStandings] = import_react21.default.useState({});
  import_react21.default.useEffect(
    function() {
      let alive = true;
      callAidosRemote("reviewStandings", { ticketId: props.ticketIdKey }, props.agentId).then((out) => {
        if (!alive) return;
        const rows = out?.rows;
        const next = {};
  ...
  return /* @__PURE__ */ import_react21.default.createElement(
    ...
    /* @__PURE__ */ import_react21.default.createElement("div", { className: "aidos-panel-body" }, props.evidence.length === 0 ? /* @__PURE__ */ import_react21.default.createElement("p", { className: "aidos-detail-note" }, "No evidence rows yet.") : /* @__PURE__ */ import_react21.default.createElement("ul", { className: "aidos-evidence-list" }, props.evidence.map((row, index) => /* @__PURE__ */ import_react21.default.createElement(
      EvidenceStrip,
      {
        ...
        standing: standings[String(row.at) + ":" + row.kind]?.standing,
...
function DetailPanel(props) {
  ...
  return /* @__PURE__ */ import_react21.default.createElement(import_react21.default.Fragment, null, /* @__PURE__ */ import_react21.default.createElement("div", { className: "aidos-detail-head" }, /* @__PURE__ */ import_react21.default.createElement(
    ...
    ticketChipLabel(ticket)
  ), /* @__PURE__ */ import_react21.default.createElement("span", { className: badge }, stateLabel(ticket.state)), /* @__PURE__ */ import_react21.default.createElement(TicketTagChips, { tags: ticket.tags ?? [] })), /* @__PURE__ */ import_react21.default.createElement("dl", { className: "aidos-facts" }, /* @__PURE__ */ import_react21.default.createElement("div", { className: "aidos-facts-row" }, /* @__PURE__ */ import_react21.default.createElement("dt", { className: "aidos-facts-label" }, "State"), /* @__PURE__ */ import_react21.default.createElement("dd", { className: "aidos-facts-value" }, stateLabel(ticket.state))), /* @__PURE__ */ import_react21.default.createElement("div", { className: "aidos-facts-row" }, /* @__PURE__ */ import_react21.default.createElement("dt", { className: "aidos-facts-label" }, "Gate"), /* @__PURE__ */ import_react21.default.createElement("dd", { className: "aidos-facts-value" }, formatGateFraction(ticket.gatePresent, ticket.gateTotal, hasCriteria(ticket)))), /* @__PURE__ */ import_react21.default.createElement("div", { className: "aidos-facts-row" }, /* @__PURE__ */ import_react21.default.createElement("dt", { className: "aidos-facts-label" }, "Confidence"), /* @__PURE__ */ import_react21.default.createElement("dd", { className: "aidos-facts-value" }, String(ringPercent(ticket.confidenceScore)) + "%", /* @__PURE__ */ import_react21.default.createElement(
    "span",
    {
  ...
  ), props.actions, props.onOpenRetire !== void 0 ? /* @__PURE__ */ import_react21.default.createElement(
    "button",
    {
      className: "aidos-btn",
...
function DetailView(props) {
  ...
  const useStoredModal = function(modal) {
    const [value, setValue] = import_react21.default.useState(() => isDetailModalOpen(agentId, ticketKey, modal));
  ...
  const [viewingEvidence, setViewingEvidenceState] = import_react21.default.useState(
    () => getViewedEvidence(agentId, ticketKey)
  );
  const setViewingEvidence = function(row) {
  ...
  ) : null, retireOpen ? /* @__PURE__ */ import_react21.default.createElement(
    RetireDialog,
    {
      open: true,
      ticketId: props.ticketIdKey,
...
// src/client/ticket-tile.tsx
function TicketTagChips(props) {
  if (props.tags.length === 0) return null;
  return /* @__PURE__ */ import_react22.default.createElement(import_react22.default.Fragment, null, props.tags.map((tag) => /* @__PURE__ */ import_react22.default.createElement(
    "span",
    {
      key: tag,
      className: "aidos-chip aidos-chip-tag",
      style: { ["--chip-hue"]: tagColor(tag) },
      "aria-label": "Tag " + tag,
      title: "Tag " + tag,
      "data-dsh-tip": ""
    },
...
function TicketTile(props) {
  const [signoffOpen, setSignoffOpen] = import_react22.default.useState(false);
  const [verifyOpen, setVerifyOpen] = import_react22.default.useState(false);
  const [sendBackOpen, setSendBackOpen] = import_react22.default.useState(false);
  const [markDoneOpen, setMarkDoneOpen] = import_react22.default.useState(false);
  const [allowlistOpen, setAllowlistOpen] = import_react22.default.useState(false);
  const [submitting, setSubmitting] = import_react22.default.useState(false);
  const controls = {
    signoffOpen,
    setSignoffOpen,
    verifyOpen,
  ...
  if (hit !== void 0 && tilePropsEqual(hit.props, props) && tileControlsEqual(hit.controls, controls)) {
    return hit.element;
  }
...
var TILE_CACHE_LIMIT = 1024;
var tileMarkupCache = /* @__PURE__ */ new Map();
function tileCacheKey(ticket) {
  const stamped = ticket.sourceSessionId;
  return (typeof stamped === "string" ? stamped : "") + ":" + String(ticket.id);
}
function tileEvidenceEqual(a, b2) {
  return a === b2 || a.length === 0 && b2.length === 0;
}
function tilePropsEqual(a, b2) {
  return a.ticket === b2.ticket && tileEvidenceEqual(a.evidence, b2.evidence) && a.selected === b2.selected && (a.active ?? false) === (b2.active ?? false) && (a.awaitingApproval ?? false) === (b2.awaitingApproval ?? false) && a.ownWorkspaceKey === b2.ownWorkspaceKey && (a.agentId ?? null) === (b2.agentId ?? null) && (a.ticketIdKey ?? null) === (b2.ticketIdKey ?? null);
}
function tileControlsEqual(a, b2) {
  return a.signoffOpen === b2.signoffOpen && a.verifyOpen === b2.verifyOpen && a.sendBackOpen === b2.sendBackOpen && a.markDoneOpen === b2.markDoneOpen && a.allowlistOpen === b2.allowlistOpen && a.submitting === b2.submitting;
}
function renderTicketTile(props, controls) {
  const ticket = props.ticket;
  ...
  const stampedSession = ticket.sourceSessionId;
  const agentId = props.agentId ?? (typeof stampedSession === "string" ? stampedSession : null);
  ...
  function openSubmitForReview() {
    ...
    setSubmitting(true);
    void submitTicketForReview(agent, key).catch((error) => {
  ...
  return /* @__PURE__ */ import_react22.default.createElement(
    ...
    /* @__PURE__ */ import_react22.default.createElement("div", { className: "aidos-tile-meta" }, /* @__PURE__ */ import_react22.default.createElement(
      "span",
      {
        className: "aidos-chip aidos-chip-id",
        style: { ["--chip-hue"]: idColor(fullTicketId(ticket)) },
    ...
    ), superseded.length > 0 ? /* @__PURE__ */ import_react22.default.createElement(
      "span",
      {
      ...
      "+" + superseded.length
    ) : null, props.awaitingApproval === true ? /* @__PURE__ */ import_react22.default.createElement(
      "span",
      {
      ...
      /* @__PURE__ */ import_react22.default.createElement(AlertCircleIcon, null)
    ) : null, /* @__PURE__ */ import_react22.default.createElement("span", { className: badge }, stateLabel(ticket.state))),
    /* @__PURE__ */ import_react22.default.createElement("h3", { className: "aidos-tile-title" }, ticket.title),
    /* @__PURE__ */ import_react22.default.createElement("p", { className: "aidos-tile-preview" }, ticket.description),
    /* @__PURE__ */ import_react22.default.createElement("div", { className: "aidos-tile-chips" }, /* @__PURE__ */ import_react22.default.createElement(
      "span",
      {
        className: "aidos-chip aidos-chip-metric aidos-chip-gate" + (gateFailed ? " aidos-chip-fail" : ""),
      ...
      /* @__PURE__ */ import_react22.default.createElement("span", { className: "aidos-chip-key" }, /* @__PURE__ */ import_react22.default.createElement(KeyholeIcon, null)),
      /* @__PURE__ */ import_react22.default.createElement("span", { className: "aidos-chip-value" }, formatGateFraction(ticket.gatePresent, ticket.gateTotal, hasCriteria(ticket)))
    ), /* @__PURE__ */ import_react22.default.createElement(EvidenceTags, { evidence: props.evidence, state: ticket.state }), /* @__PURE__ */ import_react22.default.createElement(TicketTagChips, { tags: ticket.tags ?? [] }), ticket.dependsOn?.map((ref) => /* @__PURE__ */ import_react22.default.createElement(
      "span",
      {
        key: ref,
      ...
      /* @__PURE__ */ import_react22.default.createElement("span", { className: "aidos-chip-dep-icon" }, /* @__PURE__ */ import_react22.default.createElement(ForkIcon, null)),
      ...
      /* @__PURE__ */ import_react22.default.createElement("span", { className: "aidos-chip-key" }, /* @__PURE__ */ import_react22.default.createElement(CompassIcon, null)),
      /* @__PURE__ */ import_react22.default.createElement("span", { className: "aidos-chip-value" }, ringPercent(ticket.confidenceScore) + "%")
    )),
    /* @__PURE__ */ import_react22.default.createElement(
      "div",
      {
      ...
      signoffOpen && agentId !== null ? /* @__PURE__ */ import_react22.default.createElement(
        SignoffDialog,
        {
          open: true,
      ...
      verifyOpen && agentId !== null ? /* @__PURE__ */ import_react22.default.createElement(
        VerifyModal,
        {
          ticketId: ticketIdKey,
          agentId,
      ...
      sendBackOpen && agentId !== null ? /* @__PURE__ */ import_react22.default.createElement(
        SendBackModal,
        {
          open: true,
          ticketId: ticketIdKey,
      ...
      markDoneOpen && agentId !== null ? /* @__PURE__ */ import_react22.default.createElement(
        MarkDoneModal,
        {
          open: true,
          ticketId: ticketIdKey,
      ...
      allowlistOpen && agentId !== null ? /* @__PURE__ */ import_react22.default.createElement(
        AllowlistEditor,
        {
          open: true,
          ticketId: ticket.id,
...
function TicketView(props) {
  ...
  const tiles = props.tickets.map((ticket) => /* @__PURE__ */ import_react23.default.createElement(
    TicketTile,
    {
  ...
  if (props.allTicketsCount === 0) {
    content = /* @__PURE__ */ import_react23.default.createElement("div", { className: "aidos-empty" }, /* @__PURE__ */ import_react23.default.createElement("h3", { className: "aidos-empty-title" }, "No tickets yet"), /* @__PURE__ */ import_react23.default.createElement("p", { className: "aidos-empty-note" }, "This session holds no tickets. Create the first one to start the board."), /* @__PURE__ */ import_react23.default.createElement("button", { className: "aidos-btn aidos-btn-primary", onClick: props.onCreate }, "Create a ticket"));
  } else if (props.tickets.length === 0) {
    content = /* @__PURE__ */ import_react23.default.createElement("div", { className: "aidos-empty" }, /* @__PURE__ */ import_react23.default.createElement("h3", { className: "aidos-empty-title" }, "No tickets match"), /* @__PURE__ */ import_react23.default.createElement("p", { className: "aidos-empty-note" }, "The active filters hide every ticket. Clear them to see the board."), /* @__PURE__ */ import_react23.default.createElement("button", { className: "aidos-btn", onClick: props.onClearFilters }, "Clear filters"));
  } else {
    content = /* @__PURE__ */ import_react23.default.createElement("div", { className: "aidos-board-grid" }, tiles);
  }
  const queueButton = queueButtonState(props.queueTotal ?? 0, props.agentAskCount ?? 0);
  return /* @__PURE__ */ import_react23.default.createElement("div", { className: "aidos-root" }, /* @__PURE__ */ import_react23.default.createElement("div", { className: "aidos-toolbar" }, /* @__PURE__ */ import_react23.default.createElement("span", { className: "aidos-empty-note" }, props.tickets.length + " of " + props.allTicketsCount + " tickets"), /* @__PURE__ */ import_react23.default.createElement("span", { className: "aidos-toolbar-actions" }, props.onQueue !== void 0 ? (
    /*
    ...
    /* @__PURE__ */ import_react23.default.createElement(
      ...
      /* @__PURE__ */ import_react23.default.createElement(
        "b",
        {
          className: "aidos-queue-count" + (queueButton.indicator ? " aidos-queue-count-asks" : "")
        },
        queueButton.count
      )
    )
  ) : null, props.onRetired !== void 0 && (props.retiredCount ?? 0) > 0 ? /* @__PURE__ */ import_react23.default.createElement(
    "button",
    {
      className: "aidos-btn",
    ...
    "Retired",
    /* @__PURE__ */ import_react23.default.createElement("b", { className: "aidos-queue-count" }, props.retiredCount)
  ) : null, /* @__PURE__ */ import_react23.default.createElement("button", { className: "aidos-btn", onClick: props.onPlan }, "Plan"), props.onTags !== void 0 ? /* @__PURE__ */ import_react23.default.createElement(
    "button",
    {
      className: "aidos-btn",
    ...
    "Tags",
    /* @__PURE__ */ import_react23.default.createElement("b", { className: "aidos-queue-count" }, props.tagsTotal ?? 0)
  ) : null, /* @__PURE__ */ import_react23.default.createElement("button", { className: "aidos-btn aidos-btn-primary", onClick: props.onCreate }, "Create"))), /* @__PURE__ */ import_react23.default.createElement(
    FilterPanel,
    {
  ...
  ), /* @__PURE__ */ import_react23.default.createElement("div", { className: "aidos-grid-wrap" }, content));
}
...
function CreateTicketModal(props) {
  const [title, setTitle] = import_react24.default.useState("");
  const [description, setDescription] = import_react24.default.useState("");
  const [criteria, setCriteria] = import_react24.default.useState("");
  const [tagsInput, setTagsInput] = import_react24.default.useState("");
  const [saving, setSaving] = import_react24.default.useState(false);
    ...
    try {
      ...
      if (tags.length > 0 && Number.isFinite(id)) {
        try {
          ...
          const detail = attachError instanceof AidosRemoteError ? attachError.message : String(attachError);
          showToast("Ticket created, but its tags were not attached: " + detail, "refusal");
          props.onClose();
          if (props.onCreated !== void 0) {
            props.onCreated(id);
          }
          return;
        }
  ...
  return /* @__PURE__ */ import_react24.default.createElement(
    ...
    /* @__PURE__ */ import_react24.default.createElement(
      ...
      /* @__PURE__ */ import_react24.default.createElement("div", { className: "aidos-modal-head" }, /* @__PURE__ */ import_react24.default.createElement("h3", { className: "aidos-modal-title" }, "Create a ticket"), /* @__PURE__ */ import_react24.default.createElement(
        "button",
        {
          className: "aidos-close-btn",
          onClick: () => {
      ...
      /* @__PURE__ */ import_react24.default.createElement("div", { className: "aidos-modal-form" }, /* @__PURE__ */ import_react24.default.createElement("div", { className: "aidos-modal-row" }, /* @__PURE__ */ import_react24.default.createElement("label", null, "Title"), /* @__PURE__ */ import_react24.default.createElement(
        "input",
        {
          type: "text",
          value: title,
      ...
      )), /* @__PURE__ */ import_react24.default.createElement("div", { className: "aidos-modal-row" }, /* @__PURE__ */ import_react24.default.createElement("label", null, "Description"), /* @__PURE__ */ import_react24.default.createElement(
        "textarea",
        {
          value: description,
          disabled: saving,
      ...
      )), /* @__PURE__ */ import_react24.default.createElement("div", { className: "aidos-modal-row" }, /* @__PURE__ */ import_react24.default.createElement("label", null, "Criteria"), /* @__PURE__ */ import_react24.default.createElement(
        "textarea",
        {
          value: criteria,
          disabled: saving,
      ...
      )), /* @__PURE__ */ import_react24.default.createElement("div", { className: "aidos-modal-row" }, /* @__PURE__ */ import_react24.default.createElement("label", null, "Tags"), /* @__PURE__ */ import_react24.default.createElement(
        "input",
        {
          type: "text",
          value: tagsInput,
...
function PlanMetaModal(props) {
  const [editing, setEditing] = import_react25.default.useState(null);
  const [draft, setDraft] = import_react25.default.useState("");
  const [saving, setSaving] = import_react25.default.useState(false);
  const [expanded, setExpanded] = import_react25.default.useState([]);
  ...
  function renderEditControls(key) {
    ...
    ), /* @__PURE__ */ import_react25.default.createElement("div", { className: "aidos-plan-meta-actions" }, /* @__PURE__ */ import_react25.default.createElement(
      "button",
      {
        className: "aidos-btn aidos-btn-primary",
        disabled: saving,
  ...
  function renderNamedBlock(key, label, text) {
    return /* @__PURE__ */ import_react25.default.createElement("div", { className: "aidos-plan-meta-block", key }, /* @__PURE__ */ import_react25.default.createElement("div", { className: "aidos-plan-meta-block-head" }, /* @__PURE__ */ import_react25.default.createElement("span", { className: "aidos-plan-meta-block-title" }, label), /* @__PURE__ */ import_react25.default.createElement(
      "button",
      {
        className: "aidos-btn",
        disabled: saving,
      ...
      "Edit"
    )), editing === key ? renderEditControls(key) : /* @__PURE__ */ import_react25.default.createElement("pre", { className: "aidos-plan-meta-text" }, text === "" ? "(empty)" : text));
  }
  function renderSectionBlock(position, heading, text) {
    const open = expanded.includes(position);
    return /* @__PURE__ */ import_react25.default.createElement("div", { className: "aidos-plan-meta-block", key: position }, /* @__PURE__ */ import_react25.default.createElement("div", { className: "aidos-plan-meta-block-head" }, /* @__PURE__ */ import_react25.default.createElement(
      "button",
      {
      ...
      /* @__PURE__ */ import_react25.default.createElement("span", { "aria-hidden": "true" }, open ? "\u25BE" : "\u25B8"),
      heading
    ), /* @__PURE__ */ import_react25.default.createElement(
      "button",
      {
      ...
      "Edit"
    )), editing === position || open ? editing === position ? renderEditControls(position) : /* @__PURE__ */ import_react25.default.createElement("pre", { className: "aidos-plan-meta-text" }, text === "" ? "(empty)" : text) : null);
  }
  ...
  return /* @__PURE__ */ import_react25.default.createElement(
    ...
    /* @__PURE__ */ import_react25.default.createElement(
      ...
      /* @__PURE__ */ import_react25.default.createElement("div", { className: "aidos-modal-head" }, /* @__PURE__ */ import_react25.default.createElement("h3", { className: "aidos-modal-title" }, "Plan"), /* @__PURE__ */ import_react25.default.createElement(
        "button",
        {
          className: "aidos-close-btn",
          onClick: () => {
      ...
      meta === null ? /* @__PURE__ */ import_react25.default.createElement("p", { className: "aidos-plan-meta-note" }, "This project holds no plan yet.") : /* @__PURE__ */ import_react25.default.createElement("div", { className: "aidos-plan-meta-blocks" }, renderNamedBlock("frontmatter", "Frontmatter", meta.frontmatter), renderNamedBlock("preamble", "Preamble", meta.preamble), meta.contextSections.map(
...
function ApprovalRunner(props) {
  ...
  const [index, setIndex] = import_react26.default.useState(0);
  const [values, setValues] = import_react26.default.useState(
  ...
  return /* @__PURE__ */ import_react26.default.createElement(
    ...
    /* @__PURE__ */ import_react26.default.createElement("div", { className: "aidos-runner-step" }, /* @__PURE__ */ import_react26.default.createElement("h4", { className: "aidos-runner-step-title" }, step.title), step.prompt !== void 0 ? /* @__PURE__ */ import_react26.default.createElement("p", { className: "aidos-runner-step-prompt" }, step.prompt) : null, /* @__PURE__ */ import_react26.default.createElement(StepBody, { step, value, working, onChange: update })),
    /* @__PURE__ */ import_react26.default.createElement("div", { className: "aidos-form-actions" }, /* @__PURE__ */ import_react26.default.createElement(
      "button",
      {
        className: "aidos-btn",
        disabled: working,
      ...
      "Reject"
    ), index > 0 ? /* @__PURE__ */ import_react26.default.createElement(
      "button",
      {
        className: "aidos-btn",
        disabled: working,
      ...
      "Back"
    ) : null, /* @__PURE__ */ import_react26.default.createElement("button", { className: "aidos-btn aidos-btn-primary", disabled: working, onClick: advance }, working ? "Working\u2026" : last ? "Confirm" : "Next"))
  );
}
function StepBody(props) {
  ...
  if (step.kind === "confirm" && value.kind === "confirm") {
    return /* @__PURE__ */ import_react26.default.createElement(import_react26.default.Fragment, null, /* @__PURE__ */ import_react26.default.createElement(
    ...
    ), step.criteria !== void 0 && step.criteria.length > 0 ? /* @__PURE__ */ import_react26.default.createElement("div", { className: "aidos-modal-row" }, /* @__PURE__ */ import_react26.default.createElement("label", null, "Link to a criterion (optional)"), /* @__PURE__ */ import_react26.default.createElement(
      "select",
      {
      ...
      /* @__PURE__ */ import_react26.default.createElement("option", { value: "" }, "\u2014 none \u2014"),
      step.criteria.map((criterion) => /* @__PURE__ */ import_react26.default.createElement("option", { key: criterion, value: criterion }, criterion))
    )) : null);
  }
  ...
  if (step.kind === "criteria-checklist" && value.kind === "criteria-checklist") {
    ...
    return /* @__PURE__ */ import_react26.default.createElement("ul", { className: "aidos-runner-checklist" }, step.criteria.map((criterion) => /* @__PURE__ */ import_react26.default.createElement("li", { key: criterion }, /* @__PURE__ */ import_react26.default.createElement("label", null, /* @__PURE__ */ import_react26.default.createElement(
      "input",
      {
        type: "checkbox",
    ...
    ), /* @__PURE__ */ import_react26.default.createElement("span", null, criterion)))));
  }
  if (step.kind === "dependency-picker" && value.kind === "dependency-picker") {
    const chosen = new Set(value.ticketIds);
    return /* @__PURE__ */ import_react26.default.createElement("ul", { className: "aidos-ticket-strips" }, step.candidates.map((candidate) => {
      ...
      return /* @__PURE__ */ import_react26.default.createElement(
        TicketStrip,
        {
          ...
          actions: /* @__PURE__ */ import_react26.default.createElement(
            "button",
            {
              className: "aidos-btn",
              disabled: working,
...
var ACTION_ICONS = {
  signoff: {
    icon: /* @__PURE__ */ import_react27.default.createElement(SignoffIcon, null),
    hint: "Sign off \u2014 let the agent start work on this ticket",
    tone: "signoff"
  },
  verify: {
    icon: /* @__PURE__ */ import_react27.default.createElement(VerifyIcon, null),
    hint: "Verify \u2014 check the work and attach your row",
    tone: "verify"
  },
  "mark-done": {
    icon: /* @__PURE__ */ import_react27.default.createElement(MarkDoneIcon, null),
    hint: "Mark done \u2014 close this ticket",
    tone: "done"
  },
  allowlist: {
    icon: /* @__PURE__ */ import_react27.default.createElement(AllowlistIcon, null),
...
function QueuePanel(props) {
  const [openRow, setOpenRow] = import_react27.default.useState(null);
  const [runningKey, setRunningKey] = import_react27.default.useState(
    () => getRunningApproval(props.sessionId)
  );
  const setRunning = function(entry) {
    const key = entry === null ? null : entryKey(entry);
    setRunningApproval(props.sessionId, key);
    setRunningKey(key);
  };
  const [working, setWorking] = import_react27.default.useState(false);
  const [sortKey, setSortKey] = import_react27.default.useState("suggested");
  const [activeTab, setActiveTab] = import_react27.default.useState(null);
  const [armedDismiss, setArmedDismiss] = import_react27.default.useState(null);
  const [answered, setAnswered] = import_react27.default.useState(/* @__PURE__ */ new Set());
  ...
  const active = tabs.find((tab) => tab.id === resolvedTab) ?? {
    id: resolvedTab,
    label: resolvedTab,
    entries: []
  };
  const running = runningKey === null ? null : entries.find((entry) => entryKey(entry) === runningKey) ?? null;
  ...
  if (props.error != null && props.error !== "") {
    return /* @__PURE__ */ import_react27.default.createElement("div", { className: "aidos-queue" }, /* @__PURE__ */ import_react27.default.createElement("p", { className: "aidos-queue-empty" }, "Could not load the queue: " + props.error), props.onRefresh !== void 0 ? /* @__PURE__ */ import_react27.default.createElement("button", { className: "aidos-btn", onClick: props.onRefresh }, "Retry") : null);
  }
  if (visible.length === 0) {
    return /* @__PURE__ */ import_react27.default.createElement("div", { className: "aidos-queue" }, /* @__PURE__ */ import_react27.default.createElement("p", { className: "aidos-queue-empty" }, "Nothing is waiting on you. Every ticket is either with the agent or done."));
  }
  return /* @__PURE__ */ import_react27.default.createElement("div", { className: "aidos-queue" }, /* @__PURE__ */ import_react27.default.createElement("div", { className: "aidos-queue-head" }, /* @__PURE__ */ import_react27.default.createElement("span", { className: "aidos-queue-count" }, visible.length + (visible.length === 1 ? " ask" : " asks"), suggested > 0 ? " \xB7 " + suggested + " suggested by the agent" : ""), /* @__PURE__ */ import_react27.default.createElement("label", { className: "aidos-queue-sort" }, /* @__PURE__ */ import_react27.default.createElement("span", null, "Sort"), /* @__PURE__ */ import_react27.default.createElement(
    "select",
    {
      className: "aidos-select",
      value: sortKey,
    ...
    Object.keys(QUEUE_SORT_LABELS).map((key) => /* @__PURE__ */ import_react27.default.createElement("option", { key, value: key }, QUEUE_SORT_LABELS[key]))
  ))), unmatched.length > 0 ? /* @__PURE__ */ import_react27.default.createElement("ul", { className: "aidos-queue-unmatched" }, unmatched.map((row) => /* @__PURE__ */ import_react27.default.createElement("li", { key: row.nomination.id }, "The agent suggested " + row.nomination.actionId + " but it is not shown: " + row.reason))) : null, /* @__PURE__ */ import_react27.default.createElement("div", { className: "aidos-queue-tabs", role: "tablist", "aria-label": "Queue" }, tabs.map((tab) => /* @__PURE__ */ import_react27.default.createElement(
    "button",
    {
      key: tab.id,
      type: "button",
      ...
      className: "aidos-queue-tab" + (tab.id === active.id ? " aidos-queue-tab-active" : ""),
      onClick: () => {
        setActiveTab(tab.id);
      }
    },
    /* @__PURE__ */ import_react27.default.createElement("span", { className: "aidos-queue-tab-icon", "aria-hidden": "true" }, ACTION_ICONS[QUEUE_TAB_ICON_ACTION[tab.id]]?.icon),
    /* @__PURE__ */ import_react27.default.createElement("span", { className: "aidos-queue-tab-label" }, tab.label),
    /* @__PURE__ */ import_react27.default.createElement("span", { className: "aidos-queue-tab-count" }, tab.entries.length)
  ))), active.entries.length === 0 ? /* @__PURE__ */ import_react27.default.createElement("p", { className: "aidos-queue-empty" }, QUEUE_TAB_EMPTY[active.id]) : /* @__PURE__ */ import_react27.default.createElement("ul", { className: "aidos-ticket-strips", role: "tabpanel" }, active.entries.map((entry) => /* @__PURE__ */ import_react27.default.createElement(
    TicketStrip,
    {
      key: entryKey(entry),
      ...
      meta: entry.nominationReason !== void 0 ? /* @__PURE__ */ import_react27.default.createElement("span", { className: "aidos-queue-reason" }, "the agent asks: " + entry.nominationReason) : entry.prompt,
      onOpen: () => {
        props.onOpen(entry);
      },
      actions: /* @__PURE__ */ import_react27.default.createElement(import_react27.default.Fragment, null, entry.nominationId !== void 0 && props.onDismiss !== void 0 ? /* @__PURE__ */ import_react27.default.createElement(
        "button",
        {
          type: "button",
          className: "tool-render-approval-btn tool-render-approval-reject",
          "data-armed": armedDismiss === entry.nominationId ? true : void 0,
          disabled: working,
          title: "Drop this suggestion without acting on it",
          "data-dsh-tip": "",
          onClick: () => {
            const id = entry.nominationId;
  ...
  ))), running !== null && running.actionId === "verify" && running.approvalId === void 0 ? /* @__PURE__ */ import_react27.default.createElement(
    VerifyModal,
    {
  ...
  ) : running !== null && running.actionId === "signoff" && running.approvalId === void 0 ? (
    /*
  ...
  ) : running !== null && running.actionId === "mark-done" && running.approvalId === void 0 ? (
    /*
  ...
  ) : running !== null ? /* @__PURE__ */ import_react27.default.createElement(
    ApprovalRunner,
    {
      title: running.label,
...
function queueEntriesFor(tickets, evidenceByTicket, nominations = [], approvals = []) {
  return humanQueue(
    tickets,
  ...
  for (const entry of tags) {
    ...
    if (Array.isArray(row.tickets)) {
      for (const item of row.tickets) {
        ...
        const ticket = item;
        if (typeof ticket.boardKey !== "string" || typeof ticket.id !== "number" || typeof ticket.title !== "string" || typeof ticket.state !== "string" || typeof ticket.slug !== "string" || typeof ticket.workspaceKey !== "string") {
          continue;
        }
...
function filterTagRows(rows, query) {
  const needle = query.trim().toLowerCase();
  if (needle === "") return [...rows];
  return rows.filter((row) => row.tag.toLowerCase().includes(needle));
}
function proposalsForTag(approvals, tag) {
  return approvals.filter(
    (approval) => (approval.kind === "tag-delete" || approval.kind === "tag-migrate" || approval.kind === "tag-detach") && approval.payload.tag === tag
  );
}
function proposalAction(proposal) {
  ...
  if (proposal.kind === "tag-detach") {
    ...
    return typeof ticketId === "number" || typeof ticketId === "string" ? `detach ${String(proposal.payload.tag)} from #${ticketId}` : `detach ${String(proposal.payload.tag)}`;
  }
  return `delete ${String(proposal.payload.tag)}`;
}
function TagsModal(props) {
  const [rows, setRows] = import_react28.default.useState(null);
  const [proposals, setProposals] = import_react28.default.useState([]);
  const [error, setError] = import_react28.default.useState(null);
  const [query, setQuery] = import_react28.default.useState("");
  const [selected, setSelected] = import_react28.default.useState(null);
  const [migrateTo, setMigrateTo] = import_react28.default.useState("");
  const [working, setWorking] = import_react28.default.useState(null);
  const refresh = import_react28.default.useCallback(
    function() {
      setError(null);
      callAidosRemote("workspaceTags", {}, props.sessionId).then((result) => {
        setRows(tagRowsFor(result));
      }).catch((refreshError) => {
        setError(refreshError instanceof Error ? refreshError.message : String(refreshError));
      });
      callAidosRemote("pendingApprovals", {}, props.sessionId).then((result) => {
  ...
  async function act(kind, args, label) {
    setWorking(label);
    try {
  ...
  const body = error !== null ? /* @__PURE__ */ import_react28.default.createElement("div", { className: "aidos-empty" }, /* @__PURE__ */ import_react28.default.createElement("p", { className: "aidos-empty-note" }, error), /* @__PURE__ */ import_react28.default.createElement("button", { className: "aidos-btn", onClick: refresh }, "Retry")) : rows === null ? /* @__PURE__ */ import_react28.default.createElement("div", { className: "aidos-merge-loading", role: "status" }, /* @__PURE__ */ import_react28.default.createElement("span", { className: "aidos-merge-spinner", "aria-hidden": "true" }), /* @__PURE__ */ import_react28.default.createElement("span", null, "Loading workspace tags\u2026")) : rows.length === 0 ? /* @__PURE__ */ import_react28.default.createElement("div", { className: "aidos-empty" }, /* @__PURE__ */ import_react28.default.createElement("h3", { className: "aidos-empty-title" }, "No tags yet"), /* @__PURE__ */ import_react28.default.createElement("p", { className: "aidos-empty-note" }, "Tags are created by attaching them \u2014 the agent attaches with attach_tags, and a name no ticket carries yet is created by the attach.")) : /* @__PURE__ */ import_react28.default.createElement(import_react28.default.Fragment, null, /* @__PURE__ */ import_react28.default.createElement("div", { className: "aidos-search-box" }, /* @__PURE__ */ import_react28.default.createElement(
    "input",
    {
      className: "aidos-search-input",
      type: "search",
  ...
  )), /* @__PURE__ */ import_react28.default.createElement("ul", { className: "aidos-tags-list" }, visible.map((row) => /* @__PURE__ */ import_react28.default.createElement("li", { key: row.tag, className: "aidos-tags-row" }, /* @__PURE__ */ import_react28.default.createElement(
    "button",
    {
    ...
    row.tag,
    /* @__PURE__ */ import_react28.default.createElement("b", { className: "aidos-queue-count" }, row.count)
  )))), visible.length === 0 ? /* @__PURE__ */ import_react28.default.createElement("p", { className: "aidos-empty-note" }, "No tags match this filter.") : null, selectedRow === null ? null : /* @__PURE__ */ import_react28.default.createElement("div", { className: "aidos-tags-detail" }, /* @__PURE__ */ import_react28.default.createElement("h4", { className: "aidos-tags-detail-title" }, selectedRow.tag, " \u2014 ", selectedRow.count, " ticket(s)"), selectedRow.tickets.map((ticket) => /* @__PURE__ */ import_react28.default.createElement("div", { key: ticket.boardKey, className: "aidos-tags-ticket" }, /* @__PURE__ */ import_react28.default.createElement(
    TicketStrip,
    {
      ticket: {
        id: ticket.id,
      ...
      onOpen: () => {
        props.onOpen(ticket.boardKey);
      }
    }
  ), /* @__PURE__ */ import_react28.default.createElement("span", { className: "aidos-tags-actions" }, /* @__PURE__ */ import_react28.default.createElement(
    "button",
    {
      className: "aidos-btn",
    ...
    "Detach"
  )))), /* @__PURE__ */ import_react28.default.createElement("div", { className: "aidos-tags-actions" }, /* @__PURE__ */ import_react28.default.createElement("div", { className: "aidos-search-box" }, /* @__PURE__ */ import_react28.default.createElement(
    "input",
    {
      className: "aidos-search-input",
      type: "text",
    ...
    "Delete"
  )), selectedProposals.length === 0 ? null : /* @__PURE__ */ import_react28.default.createElement("div", { className: "aidos-tags-proposals" }, /* @__PURE__ */ import_react28.default.createElement("h4", { className: "aidos-tags-detail-title" }, "Agent proposals awaiting approval"), selectedProposals.map((proposal) => /* @__PURE__ */ import_react28.default.createElement("div", { key: proposal.id, className: "aidos-tags-proposal" }, /* @__PURE__ */ import_react28.default.createElement("span", null, proposalAction(proposal)), typeof proposal.payload.reason === "string" && proposal.payload.reason !== "" ? /* @__PURE__ */ import_react28.default.createElement("span", { className: "aidos-empty-note" }, " \u2014 ", proposal.payload.reason) : null, /* @__PURE__ */ import_react28.default.createElement("span", { className: "aidos-tags-actions" }, /* @__PURE__ */ import_react28.default.createElement(
    "button",
    {
      className: "aidos-btn aidos-btn-primary",
      disabled: working !== null,
  ...
  return /* @__PURE__ */ import_react28.default.createElement(ModalShell, { title: "Tags", wide: true, onClose: props.onClose }, body);
}
...
function ToastRow(props) {
  ...
  return /* @__PURE__ */ import_react29.default.createElement("div", { className: "aidos-toast aidos-toast-" + toast.kind }, /* @__PURE__ */ import_react29.default.createElement("span", { className: "aidos-toast-text" }, toast.text), /* @__PURE__ */ import_react29.default.createElement(
    "button",
    {
...
function ToastContainer() {
  const [toasts2, setToasts] = import_react29.default.useState([]);
  ...
  return /* @__PURE__ */ import_react29.default.createElement("div", { className: "aidos-toast-stack" }, toasts2.map(function(toast) {
    return /* @__PURE__ */ import_react29.default.createElement(ToastRow, { key: toast.id, toast });
  }));
}
...
var boardVersions = /* @__PURE__ */ new Map();
var MERGE_PULL_DEBOUNCE_MS = 300;
...
var ROW_CACHE_LIMIT = 4096;
var stableRowCache = /* @__PURE__ */ new Map();
function stabilizeBoardRows(rows, sessionId, foreign) {
  return rows.map((raw) => {
    const owner = foreign ? raw.sourceSessionId ?? "" : sessionId;
    const key = owner + ":" + String(raw.id);
    const hash = JSON.stringify(raw) + "|" + owner + "|" + String(foreign);
    const hit = stableRowCache.get(key);
    if (hit !== void 0 && hit.hash === hash) return hit.row;
    const row = foreign ? { ...raw } : { ...raw, sourceSessionId: sessionId, foreign: false };
    if (stableRowCache.size >= ROW_CACHE_LIMIT) stableRowCache.clear();
    stableRowCache.set(key, { hash, row });
    return row;
  });
}
var EVIDENCE_CACHE_LIMIT = 4096;
var stableEvidenceCache = /* @__PURE__ */ new Map();
function stabilizeEvidence(evidence) {
  const out = {};
  for (const [key, rows] of Object.entries(evidence)) {
    const hash = JSON.stringify(rows);
    const hit = stableEvidenceCache.get(key);
    if (hit !== void 0 && hit.hash === hash) {
      out[key] = hit.rows;
      continue;
    }
...
function restoreFilter(workspaceKey, tickets) {
  try {
    ...
    const tags = Array.isArray(parsed.tags) && parsed.tags.every((tag) => typeof tag === "string") ? [...parsed.tags] : void 0;
    return {
      projectIds,
      stateIds,
      sortKey,
...
function ticketRefFromSearch(search) {
...
function LocalTicketView(props) {
  const [retryNonce, setRetryNonce] = import_react30.default.useState(0);
  import_react30.default.useEffect(function() {
...
function ProjectionReader(props) {
  ...
  const [merge, setMergeState] = import_react30.default.useState(() => getMerge(sessionId));
  const [mergePending, setMergePending] = import_react30.default.useState(() => isMergePulling(sessionId) && getMerge(sessionId) === null);
  ...
  import_react30.default.useEffect(function() {
    ...
    const pull = async function() {
      try {
        ...
        const result = await callAidosRemote(
          "workspaceTickets",
          sinceVersion === void 0 ? {} : { sinceVersion },
          sessionId
        );
        const labels = result.workspaceLabels;
        if (labels !== void 0) {
          for (const [key, label] of Object.entries(labels)) {
            rememberWorkspaceLabel(key, label);
          }
        }
        const gated = result.unchanged === true;
        if (!gated) {
  ...
  const ownRows = stabilizeBoardRows(
    Object.values(ticketsProjection ?? {}),
    sessionId,
    false
  );
  const foreignRows = merge !== null ? stabilizeBoardRows(
  ...
  const rawEvidence = stabilizeEvidence({
    ...foreignEvidence,
    ...ownEvidence
  });
  ...
  const [applied, setAppliedStateLocal] = import_react30.default.useState(function() {
    return cloneAppliedState(DEFAULT_APPLIED);
  });
  const [selectedKeyRaw, setSelectedKeyRaw] = import_react30.default.useState(function() {
    const stored = getSelection(sessionId);
    return stored === null ? null : asBoardKey(stored);
  });
  const selectedKey = selectedKeyRaw;
  const setSelectedKey = import_react30.default.useCallback(
    function(next) {
      const leaving = getHeldTicket(sessionId);
  ...
  const useStoredModal = function(modal) {
    const [value, setValue] = import_react30.default.useState(() => isModalOpen(sessionId, modal));
    const set = function(open) {
  ...
  const [nominations, setNominations] = import_react30.default.useState([]);
  const [approvals, setApprovals] = import_react30.default.useState([]);
  ...
  const [queueError, setQueueError] = import_react30.default.useState(null);
  const refreshNominations = import_react30.default.useCallback(
    function() {
      setQueueError(null);
  ...
  const [errorTimedOut, setErrorTimedOut] = import_react30.default.useState(false);
  const deepLinkHandled = import_react30.default.useRef(false);
  const restoredRef = import_react30.default.useRef(false);
  const layoutRef = import_react30.default.useRef(null);
  ...
  import_react30.default.useEffect(function() {
    ...
    return function() {
      ...
      logWarn(
        `#100 ProjectionReader UNMOUNTING; ticket param present=${had}` + (had ? " -> KEPT (round 4: it is what restores the selection after a reload)" : "")
      );
  ...
  const error = errorTimedOut && !loaded ? /* @__PURE__ */ import_react30.default.createElement("div", { className: "aidos-error" }, /* @__PURE__ */ import_react30.default.createElement("span", null, "The board projection is unavailable. Retry to re-read it."), /* @__PURE__ */ import_react30.default.createElement("button", { className: "aidos-btn", onClick: props.onRetry }, "Retry")) : null;
  const filtered = filterTickets(liveTickets, applied);
  const tagsTotal = (function() {
    const seen = /* @__PURE__ */ new Set();
  ...
  const lastSelected = import_react30.default.useRef(null);
  ...
  const lastLogged = import_react30.default.useRef("");
  const shape = `${resolution.reason}|sel=${selectedKey ?? "-"}|rows=${rawTickets.length}|own=${ownRows.length}|foreign=${foreignRows.length}|ref=${lastSelected.current === null ? "null" : "held"}|store=${getHeldTicket(sessionId) === null ? "null" : "held"}`;
  ...
  if (resolution.reason === "resolved" || resolution.reason === "reanchored" || resolution.reason === "held") {
  ...
  const [evidenceCollapsed, setEvidenceCollapsed] = import_react30.default.useState(function() {
  ...
  const absentNotice = resolution.absent ? /* @__PURE__ */ import_react30.default.createElement("div", { className: "aidos-detail-absent", role: "status" }, "This ticket is not on the board right now. You are seeing the last version that loaded. Close the panel to return to the grid.") : null;
  const panelKey = selectedTicket === null ? null : fullTicketId(selectedTicket);
  const lastPanelKey = import_react30.default.useRef(null);
  ...
  const detailPanel = selectedTicket === null ? null : /* @__PURE__ */ import_react30.default.createElement(import_react30.default.Fragment, null, absentNotice, /* @__PURE__ */ import_react30.default.createElement(
    DetailView,
    {
      key: panelKey,
  ...
  const createModal = /* @__PURE__ */ import_react30.default.createElement(
    CreateTicketModal,
    {
      open: createOpen,
      onClose: () => {
  ...
  if (error !== null) {
    ...
    body = /* @__PURE__ */ import_react30.default.createElement("div", { className: "aidos-skeleton-grid" }, [0, 1, 2, 3, 4, 5].map((index) => /* @__PURE__ */ import_react30.default.createElement("div", { className: "aidos-skeleton-tile", key: index })));
  } else if (mergeLoading) {
    body = /* @__PURE__ */ import_react30.default.createElement("div", { className: "aidos-merge-loading", role: "status" }, /* @__PURE__ */ import_react30.default.createElement("span", { className: "aidos-merge-spinner", "aria-hidden": "true" }), /* @__PURE__ */ import_react30.default.createElement("span", null, "Loading workspace tickets\u2026"));
  } else {
    const queueEntries = queueEntriesFor(liveTickets, rawEvidence, nominations, approvals);
    body = /* @__PURE__ */ import_react30.default.createElement(
      TicketView,
      {
        ownWorkspaceKey,
  ...
  const queueModal = queueOpen ? /* @__PURE__ */ import_react30.default.createElement(
    ModalShell,
    {
  ...
  const retiredModal = retiredOpen ? /* @__PURE__ */ import_react30.default.createElement(
    ModalShell,
    {
  ...
  const tagsModal = tagsOpen ? /* @__PURE__ */ import_react30.default.createElement(
    TagsModal,
    {
      sessionId,
      onOpen: (key) => {
  ...
  const planModal = /* @__PURE__ */ import_react30.default.createElement(
    PlanMetaModal,
    {
      open: planOpen,
  ...
  return /* @__PURE__ */ import_react30.default.createElement(import_react30.default.Fragment, null, /* @__PURE__ */ import_react30.default.createElement("div", { className: "aidos-layout", ref: layoutRef, "data-conversation-composer-overlay": "" }, body, detailPanel), createModal, planModal, queueModal, retiredModal, tagsModal, /* @__PURE__ */ import_react30.default.createElement(ToastContainer, null));
}
...
function nameBadgeColors(name2, isError) {
  if (isError) {
    return {
...
function Leading({ state, open }) {
  void state;
  return /* @__PURE__ */ import_react31.default.createElement(ToolRenderChevron, { open });
}
function ScratchRow({
  title,
  icon,
  ...
  const expandable = body !== null || inspect !== void 0;
  const open = expanded && expandable;
  const showsError = state === "error" && errorSummary !== void 0;
  const shown = showsError ? errorSummary : summary;
  const linked = !showsError && path !== void 0 && openFile !== void 0;
  const openIt = () => {
    if (path !== void 0 && openFile !== void 0) openFile(path);
  };
  return /* @__PURE__ */ import_react31.default.createElement(
    "div",
    {
      ...
      "data-stopped": state === "stopped" || void 0
    },
    /* @__PURE__ */ import_react31.default.createElement(
      ...
      expandable ? /* @__PURE__ */ import_react31.default.createElement(Leading, { state, open }) : null,
      /* @__PURE__ */ import_react31.default.createElement("span", { className: "tool-render-name-badge", style: nameBadgeColors(title, state === "error") }, /* @__PURE__ */ import_react31.default.createElement("span", { className: "tool-render-name-badge-icon" }, icon), /* @__PURE__ */ import_react31.default.createElement("span", { className: "tool-render-name-badge-text", title, "data-dsh-tip": "" }, title)),
      /* @__PURE__ */ import_react31.default.createElement("span", { className: "tool-render-sep", "aria-hidden": "true" }),
      linked ? /* @__PURE__ */ import_react31.default.createElement(
        "span",
        {
    ...
    open ? /* @__PURE__ */ import_react31.default.createElement("div", { className: "tool-render-body" }, body, inspect !== void 0 ? /* @__PURE__ */ import_react31.default.createElement("button", { type: "button", className: "tool-render-inspect", onClick: inspect }, /* @__PURE__ */ import_react31.default.createElement(InspectIcon, null), "Inspect") : null) : null
  );
}
...
function bodyOf(text, isError) {
  ...
  return /* @__PURE__ */ import_react31.default.createElement("pre", { className: "tool-render-output", "tool-render-error": isError ? true : void 0 }, text);
}
function readBody(text, path) {
  return codeBlock(numberedReadRows(text, readStartLine(null, text)), path);
}
function codeBlock(rows, path) {
  const language = languageFor(path ?? "");
  const width = gutterWidth(rows.map((row) => row.number));
  return /* @__PURE__ */ import_react31.default.createElement("div", { className: "tool-render-code" }, rows.map((row, index) => /* @__PURE__ */ import_react31.default.createElement("div", { className: "tool-render-code-row", key: index }, /* @__PURE__ */ import_react31.default.createElement("span", { className: "tool-render-gutter", "aria-hidden": "true", style: { width } }, row.number === null ? "" : String(row.number)), /* @__PURE__ */ import_react31.default.createElement(
    "code",
    {
...
function writeBody(content, path) {
  return /* @__PURE__ */ import_react31.default.createElement("div", { className: "tool-render-write" }, /* @__PURE__ */ import_react31.default.createElement("div", { className: "tool-render-write-note" }, "No earlier version on record; new content below"), codeBlock(numberedReadRows(content, 1), path));
}
function ScratchReadRow(props) {
  const row = useRow(props, "Read");
  const { args, state, errorText: errorText2 } = row;
  ...
  return /* @__PURE__ */ import_react31.default.createElement(ScratchRow, { ...row, icon: /* @__PURE__ */ import_react31.default.createElement(CompassIcon, null), title: "Scratch read", body });
}
function ScratchWriteRow(props) {
  ...
  const body = state === "error" || written === null || written === "" ? bodyOf(written, state === "error") : writeBody(written, row.path);
  return /* @__PURE__ */ import_react31.default.createElement(ScratchRow, { ...row, icon: /* @__PURE__ */ import_react31.default.createElement(PencilIcon, null), title: "Scratch write", body });
}
...
+ ${newText ?? ""}` : unwrapScratchResult(resultTextOf(props.block));
  ...
  return /* @__PURE__ */ import_react31.default.createElement(
    ...
    {
      ...row,
      icon: /* @__PURE__ */ import_react31.default.createElement(PencilIcon, null),
      title: "Scratch edit",
...
function ScratchMkdirRow(props) {
  ...
  const { state, errorText: errorText2 } = row;
  return /* @__PURE__ */ import_react31.default.createElement(
    ScratchRow,
    {
      ...row,
      icon: /* @__PURE__ */ import_react31.default.createElement(ForkIcon, null),
      title: "Scratch mkdir",
...
function expandableFact(label, text, options) {
...
function ticketFacts(result) {
  ...
  if (allowlist.length > 0) {
    facts.push(expandableFact("Allowlist", allowlist.map((p) => String(p)).join(" \xB7 ")));
  }
  const dependsOn = asArray(ticket.dependsOn);
  if (dependsOn.length > 0) {
    facts.push(expandableFact("Depends on", dependsOn.map((d) => String(d)).join(" \xB7 ")));
  }
...
function ticketCaptionOf(result) {
  const ticket = asRecord(result?.ticket);
...
function ticketTables(result) {
  return asArray(result?.tickets).map((row) => asRecord(row)).filter((row) => row !== null).map((row) => ticketTableOf(row));
}
...
function evidenceRecords(result) {
  return asArray(result?.evidence).map((row) => asRecord(row)).filter((row) => row !== null).map((row, position) => ({
...
function evidenceComments(result) {
  return asArray(result?.comments).map((row) => asRecord(row)).filter((row) => row !== null).map((row) => ({
    author: asText(row.author) ?? "user",
    at: typeof row.at === "number" ? row.at : void 0,
    body: asText(row.body) ?? ""
  })).filter((row) => row.body !== "");
}
function boardQuerySummary(args) {
  if (args === null) return "all tickets";
  const parts = [];
  const rawStates = Array.isArray(args.stateIds) ? args.stateIds : [args.stateIds];
  const states = rawStates.map((value) => asText(value)).filter((value) => value !== null && value !== "");
  if (states.length > 0) parts.push(states.join("|"));
  const search = asText(args.search);
  if (search !== null && search !== "") parts.push(`"${search}"`);
  const projectIds = asArray(args.projectIds).map((value) => asText(value)).filter((value) => value !== null && value !== "");
  ...
  return parts.length === 0 ? "all tickets" : parts.join(" \xB7 ");
}
var SET_TICKET_ADDRESSING = /* @__PURE__ */ new Set(["ticketId", "projectId"]);
var MARKDOWN_FIELDS = /* @__PURE__ */ new Set(["description", "body"]);
...
var MOVE_TITLE_MAX = 72;
function moveTicketSummary(label, to, stateText) {
  const title = oneLine(label ?? "move", MOVE_TITLE_MAX);
  if (to === null || to === "") return { title, state: null, text: title };
  const shown = stateText === void 0 ? to : stateText(to);
...
function allowlistFacts(args, result) {
  return allowlistPaths(args, result).map(
    (entry) => expandableFact(entry.created ? "will be created" : "exists", entry.path)
  );
}
function moveFacts(result) {
...
function planImportFacts(result) {
  const facts = [];
...
function findTicketsTabButton(root) {
...
function planImportSummary(file, imported, deleted) {
  let out = file;
  if (imported !== void 0) {
    out += " \xB7 " + imported + (imported === 1 ? " ticket" : " tickets");
  }
  if (deleted === false) out += " \xB7 file kept";
  else if (deleted === true) out += " \xB7 file deleted";
  return out;
}
...
function modalTicketFor(merge, ticketId) {
  if (merge === null || ticketId === null || ticketId === void 0 || ticketId === "") {
    return null;
  }
  ...
  const ticketsByKey = /* @__PURE__ */ new Map();
  const remember = (rememberKey, view) => {
...
function AidosRow(props) {
  ...
  const [peekOpen, setPeekOpen] = import_react32.default.useState(false);
  const [evidenceCollapsed, setEvidenceCollapsed] = import_react32.default.useState(false);
  const [activationNotice, setActivationNotice] = import_react32.default.useState(null);
  ...
  const rich = props.summaryNode !== void 0 && props.errorSummary === void 0 ? props.summaryNode : null;
  const canSelect = props.ticketId !== null && props.ticketId !== void 0 && props.sessionId !== void 0;
  import_react32.default.useEffect(() => {
    if (props.ticketId === null || props.ticketId === void 0) return;
    console.info(
      `[aidos] toolview ticket=${props.ticketId} sessionId=${props.sessionId ?? "MISSING"} useProjection=${typeof props.useProjection} canSelect=${canSelect}`
    );
  ...
  const modalTicket = modalTicketFor(
    props.sessionId === void 0 ? null : getMerge(props.sessionId),
    props.ticketId
  );
  return /* @__PURE__ */ import_react32.default.createElement("div", { className: "tool-render-card", "data-error": props.state === "error" || void 0 }, /* @__PURE__ */ import_react32.default.createElement(
    "div",
    {
      className: "tool-render-row",
    ...
    /* @__PURE__ */ import_react32.default.createElement(ToolRenderChevron, { open, disabled: !expandable }),
    /* @__PURE__ */ import_react32.default.createElement("span", { className: "tool-render-name-badge", style: nameBadgeColors(props.title, props.state === "error") }, /* @__PURE__ */ import_react32.default.createElement("span", { className: "tool-render-name-badge-icon" }, props.icon), /* @__PURE__ */ import_react32.default.createElement("span", { className: "tool-render-name-badge-text", title: props.title, "data-dsh-tip": "" }, props.title)),
    /* @__PURE__ */ import_react32.default.createElement("span", { className: "tool-render-sep", "aria-hidden": "true" }),
    select !== void 0 && props.errorSummary === void 0 ? /* @__PURE__ */ import_react32.default.createElement(
      "span",
      {
  ...
  ), open ? /* @__PURE__ */ import_react32.default.createElement("div", { className: "tool-render-body" }, body, footer !== null ? /* @__PURE__ */ import_react32.default.createElement("div", { className: "aidos-tool-footer" }, footer) : null) : null, peekOpen ? /* @__PURE__ */ import_react32.default.createElement(
    ModalShell,
    {
      title: modalTicket !== null ? `#${modalTicket.ticket.id} ${modalTicket.ticket.title}` : "Ticket",
      wide: modalTicket !== null,
      onClose: () => setPeekOpen(false)
    },
    modalTicket !== null && props.sessionId !== void 0 ? /* @__PURE__ */ import_react32.default.createElement(import_react32.default.Fragment, null, /* @__PURE__ */ import_react32.default.createElement(
      DetailView,
      {
        ...
        onJump: (key) => {
          setSelection(props.sessionId, key);
          setPeekOpen(false);
        }
      }
    )) : peeked !== null ? /* @__PURE__ */ import_react32.default.createElement(import_react32.default.Fragment, null, /* @__PURE__ */ import_react32.default.createElement(TicketStrip, { ticket: peeked }), peeked.descriptionFull !== void 0 ? /* @__PURE__ */ import_react32.default.createElement(
      "div",
      {
        className: "aidos-md aidos-ticket-peek-description",
        dangerouslySetInnerHTML: { __html: renderMarkdownSafe(peeked.descriptionFull) }
      }
    ) : null, /* @__PURE__ */ import_react32.default.createElement("div", { className: "aidos-ticket-peek-actions" }, canSelect ? /* @__PURE__ */ import_react32.default.createElement(
      "button",
      {
        type: "button",
        className: "tool-render-approval-btn tool-render-approval-approve",
      ...
      /* @__PURE__ */ import_react32.default.createElement(PopOutIcon, null),
      " Open on board"
    ) : null), activationNotice !== null ? /* @__PURE__ */ import_react32.default.createElement("p", { className: "aidos-ticket-peek-note", role: "status" }, activationNotice) : null) : /* @__PURE__ */ import_react32.default.createElement("p", { className: "aidos-ticket-peek-empty" }, "#" + (props.ticketId ?? "?") + " isn't in this session's own board yet, or belongs to another session. Open the Tickets tab to look it up there.")
  ) : null);
}
function Facts({ facts }) {
  if (facts.length === 0) return null;
  return /* @__PURE__ */ import_react32.default.createElement("dl", { className: "aidos-tool-facts" }, facts.map((fact, index) => (
    /*
     ...
     */
    /* @__PURE__ */ import_react32.default.createElement(import_react32.default.Fragment, { key: fact.label + ":" + index }, /* @__PURE__ */ import_react32.default.createElement("dt", null, fact.label), /* @__PURE__ */ import_react32.default.createElement(FactValue, { fact }))
  )));
}
function FactValue({ fact }) {
  const expandable = fact.full !== void 0 && fact.full !== "";
  if (!expandable) {
    return /* @__PURE__ */ import_react32.default.createElement("dd", { title: fact.value, "data-dsh-tip": "" }, fact.value);
  }
  return /* @__PURE__ */ import_react32.default.createElement(ExpandableValue, { fact, as: "dd" });
}
function ExpandableValue({ fact, as }) {
  const [expanded, setExpanded] = import_react32.default.useState(fact.open === true);
  const Wrapper = as;
  return /* @__PURE__ */ import_react32.default.createElement(
    Wrapper,
    {
      className: as === "span" ? "aidos-tool-fact-value" : void 0,
      "data-expanded": expanded ? true : void 0
    },
    expanded ? /* @__PURE__ */ import_react32.default.createElement(ExpandedFact, { fact }) : /* @__PURE__ */ import_react32.default.createElement("span", { className: "aidos-tool-fact-clipped", title: fact.value, "data-dsh-tip": "" }, fact.value),
    /* @__PURE__ */ import_react32.default.createElement(
      "button",
      {
...
function ListValue({ fact }) {
  if (fact.full === void 0 || fact.full === "") {
    return /* @__PURE__ */ import_react32.default.createElement("span", { className: "aidos-tool-list-text", title: fact.value, "data-dsh-tip": "" }, fact.value);
  }
  return (
    /*
     ...
     */
    /* @__PURE__ */ import_react32.default.createElement("span", { className: "aidos-tool-list-text aidos-tool-list-value" }, /* @__PURE__ */ import_react32.default.createElement(ExpandableValue, { fact, as: "span" }))
  );
}
function ExpandedFact({ fact }) {
  const text = fact.full ?? fact.value;
  if (fact.markdown !== true) {
    return /* @__PURE__ */ import_react32.default.createElement("span", { className: "aidos-tool-fact-full" }, text);
  }
  return /* @__PURE__ */ import_react32.default.createElement(
    "div",
    {
...
function FactsStack({
  ...
  return /* @__PURE__ */ import_react32.default.createElement("div", { className: "aidos-tool-stack" }, tables.map((table) => /* @__PURE__ */ import_react32.default.createElement("section", { className: "aidos-tool-table", key: table.id }, /* @__PURE__ */ import_react32.default.createElement(
    TicketCaption,
    {
      id: table.id,
      state: table.state,
  ...
  ), /* @__PURE__ */ import_react32.default.createElement(Facts, { facts: table.facts }))));
}
function TicketCaption({
  ...
  return /* @__PURE__ */ import_react32.default.createElement("header", { className: "aidos-tool-table-head" }, onSelect !== void 0 ? /* @__PURE__ */ import_react32.default.createElement(
    "button",
    {
    ...
    "#" + id
  ) : /* @__PURE__ */ import_react32.default.createElement("span", { className: "aidos-tool-table-id" }, "#" + id), state === "" ? null : /* @__PURE__ */ import_react32.default.createElement("span", { className: "aidos-tool-list-tag" }, state), /* @__PURE__ */ import_react32.default.createElement("span", { className: "aidos-tool-table-title", title, "data-dsh-tip": "" }, title));
}
function ticketCaption(result) {
  const caption = ticketCaptionOf(result);
  if (caption === null) return null;
  return /* @__PURE__ */ import_react32.default.createElement(TicketCaption, { id: caption.id, state: caption.state, title: caption.title });
}
function TextBody({ text, isError }) {
  return /* @__PURE__ */ import_react32.default.createElement("pre", { className: "tool-render-output", "tool-render-error": isError === true ? true : void 0 }, text);
}
function errorBody(errorText2) {
  if (errorText2 === null || errorText2 === "") return null;
  const envelope = parseErrorEnvelope(errorText2);
  if (envelope === null) return /* @__PURE__ */ import_react32.default.createElement(TextBody, { text: errorText2, isError: true });
  const facts = [];
  ...
  if (envelope.message === null && facts.length === 0) {
    return /* @__PURE__ */ import_react32.default.createElement(TextBody, { text: errorText2, isError: true });
  }
  return /* @__PURE__ */ import_react32.default.createElement(import_react32.default.Fragment, null, envelope.message === null ? null : /* @__PURE__ */ import_react32.default.createElement("p", { className: "aidos-tool-message" }, envelope.message), /* @__PURE__ */ import_react32.default.createElement(Facts, { facts }));
}
function useAidosRow(props) {
  ...
  const errorText2 = state === "error" ? errorTextOf(block) : null;
  const errorSummary = errorText2 !== null && errorText2 !== "" ? firstLineOfError(errorText2) : void 0;
  ...
  return { args, state: shownState, result, resultText, ticketId, errorText: errorText2, errorSummary };
}
function fallbackBody(resultText) {
  return resultText === null || resultText === "" ? null : /* @__PURE__ */ import_react32.default.createElement(TextBody, { text: resultText, isError: false });
}
function AttachEvidenceRow(props) {
  ...
  const body = /* @__PURE__ */ import_react32.default.createElement(import_react32.default.Fragment, null, errorText2 !== null && errorText2 !== "" ? errorBody(errorText2) : kind !== void 0 ? /* @__PURE__ */ import_react32.default.createElement("ul", { className: "aidos-evidence-list" }, /* @__PURE__ */ import_react32.default.createElement(
    EvidenceStrip,
    {
      row: {
  ...
  )) : null, evidenceViewer.viewer);
  return /* @__PURE__ */ import_react32.default.createElement(
    AidosRow,
    {
      icon: /* @__PURE__ */ import_react32.default.createElement(SignoffIcon, null),
      title: "Evidence",
...
function MoveTicketRow(props) {
  const { args, state, result, resultText, ticketId, errorText: errorText2, errorSummary } = useAidosRow(props);
  const to = typeof args?.to === "string" ? args.to : null;
  const label = ticketLabel(props.sessionId, ticketId);
  const facts = [...moveFacts(result), ...ticketFacts(result)];
  const body = errorText2 !== null && errorText2 !== "" ? errorBody(errorText2) : facts.length > 0 ? /* @__PURE__ */ import_react32.default.createElement(Facts, { facts }) : result === null ? fallbackBody(resultText) : null;
  const parts = moveTicketSummary(label, to, stateLabel);
  const summaryNode = parts.state === null ? void 0 : /* @__PURE__ */ import_react32.default.createElement(import_react32.default.Fragment, null, /* @__PURE__ */ import_react32.default.createElement("span", { className: "aidos-move-title", title: label ?? parts.title, "data-dsh-tip": "" }, parts.title), /* @__PURE__ */ import_react32.default.createElement("span", { className: "aidos-move-arrow", "aria-hidden": "true" }, "\u2192"), /* @__PURE__ */ import_react32.default.createElement("span", { className: badgeClass(parts.state) }, stateLabel(parts.state)));
  return /* @__PURE__ */ import_react32.default.createElement(
    AidosRow,
    {
      icon: /* @__PURE__ */ import_react32.default.createElement(ForkIcon, null),
      title: "Move ticket",
      summary: parts.text,
      summaryNode,
      state,
...
function SetTicketRow(props) {
  ...
  const body = errorText2 !== null && errorText2 !== "" ? errorBody(errorText2) : fields.length > 0 ? /* @__PURE__ */ import_react32.default.createElement(Facts, { facts: fields }) : null;
  return /* @__PURE__ */ import_react32.default.createElement(
    AidosRow,
    {
      icon: /* @__PURE__ */ import_react32.default.createElement(PencilIcon, null),
...
function GetTicketRow(props) {
  const { state, result, resultText, ticketId, errorText: errorText2, errorSummary } = useAidosRow(props);
  const facts = ticketFacts(result);
  const evidence = ticketEvidence(result);
  const evidenceViewer = useEvidenceViewer();
  const body = errorText2 !== null && errorText2 !== "" ? errorBody(errorText2) : facts.length === 0 && evidence.length === 0 ? result === null ? fallbackBody(resultText) : null : /* @__PURE__ */ import_react32.default.createElement(import_react32.default.Fragment, null, ticketCaption(result), /* @__PURE__ */ import_react32.default.createElement(Facts, { facts }), evidence.length > 0 ? /* @__PURE__ */ import_react32.default.createElement("ul", { className: "aidos-evidence-list" }, evidence.map((row, index) => /* @__PURE__ */ import_react32.default.createElement(
    EvidenceStrip,
    {
      key: index,
      row: {
  ...
  return /* @__PURE__ */ import_react32.default.createElement(
    AidosRow,
    {
      icon: /* @__PURE__ */ import_react32.default.createElement(CompassIcon, null),
      title: "Read ticket",
...
function GetTicketsRow(props) {
  const { args, state, result, resultText, errorText: errorText2, errorSummary } = useAidosRow(props);
  ...
  const body = errorText2 !== null && errorText2 !== "" ? errorBody(errorText2) : tables.length === 0 ? result === null ? fallbackBody(resultText) : null : /* @__PURE__ */ import_react32.default.createElement(
    FactsStack,
    {
      tables,
      onSelect: props.sessionId === void 0 ? void 0 : (id) => {
        setSelection(props.sessionId, asBoardKey(id));
      }
    }
  );
  return /* @__PURE__ */ import_react32.default.createElement(
    AidosRow,
    {
      icon: /* @__PURE__ */ import_react32.default.createElement(KeyholeIcon, null),
      title: "Read board",
      summary,
      state,
      body,
...
function RequestAllowlistRow(props) {
  ...
  const body = errorText2 !== null && errorText2 !== "" ? errorBody(errorText2) : paths.length === 0 ? null : /* @__PURE__ */ import_react32.default.createElement(import_react32.default.Fragment, null, /* @__PURE__ */ import_react32.default.createElement(Facts, { facts: paths }), props.sessionId !== void 0 && ticketId !== null ? /* @__PURE__ */ import_react32.default.createElement(
  ...
  return /* @__PURE__ */ import_react32.default.createElement(
    AidosRow,
    {
      icon: /* @__PURE__ */ import_react32.default.createElement(AllowlistIcon, null),
...
function SuggestActionsRow(props) {
  ...
  const summary = lines.length === 0 ? "nothing" : lines.length === 1 ? ticketLabel(props.sessionId, lines[0].ticketId) ?? "#" + lines[0].ticketId : lines.length + " tickets";
  const body = errorText2 !== null && errorText2 !== "" ? errorBody(errorText2) : lines.length === 0 ? null : /* @__PURE__ */ import_react32.default.createElement("ul", { className: "aidos-tool-list" }, lines.map((line) => /* @__PURE__ */ import_react32.default.createElement("li", { key: line.ticketId + ":" + line.actionId }, /* @__PURE__ */ import_react32.default.createElement("span", { className: "aidos-tool-list-key" }, "#", line.ticketId), /* @__PURE__ */ import_react32.default.createElement("span", { className: "aidos-tool-list-tag" }, line.actionId), /* @__PURE__ */ import_react32.default.createElement(ListValue, { fact: expandableFact("reason", line.reason) }), props.sessionId !== void 0 && isInlineActionId(line.actionId) ? /* @__PURE__ */ import_react32.default.createElement(
    InlineTicketAction,
    {
      sessionId: props.sessionId,
  ...
  return /* @__PURE__ */ import_react32.default.createElement(
    AidosRow,
    {
      icon: /* @__PURE__ */ import_react32.default.createElement(AlertCircleIcon, null),
...
function PlanRow(props) {
  ...
  return /* @__PURE__ */ import_react32.default.createElement(
    AidosRow,
    {
      icon: /* @__PURE__ */ import_react32.default.createElement(PopOutIcon, null),
      title: "Export plan",
      summary,
      state,
      body: text === null || text === "" ? null : /* @__PURE__ */ import_react32.default.createElement(TextBody, { text, isError: errorText2 !== null && state === "error" }),
      errorSummary
    }
  );
}
  ...
  return /* @__PURE__ */ import_react32.default.createElement(
    AidosRow,
    {
      ...
      summary: planImportSummary(
        file,
        typeof imported === "number" ? imported : void 0,
        typeof result?.deleted === "boolean" ? result.deleted : void 0
      ),
      state,
      body,
      errorSummary
...
function PlanMetaSetRow(props) {
  ...
  const body = errorText2 !== null && errorText2 !== "" ? errorBody(errorText2) : written.length > 0 ? /* @__PURE__ */ import_react32.default.createElement(Facts, { facts: written }) : null;
  return /* @__PURE__ */ import_react32.default.createElement(
    AidosRow,
    {
      icon: /* @__PURE__ */ import_react32.default.createElement(PencilIcon, null),
...
function GetEvidenceRow(props) {
  ...
  const summary = index === null ? label + " \xB7 evidence" : label + " \xB7 evidence [" + String(index) + "]";
  const body = errorText2 !== null && errorText2 !== "" ? errorBody(errorText2) : records.length === 0 && comments.length === 0 ? result === null ? fallbackBody(resultText) : null : /* @__PURE__ */ import_react32.default.createElement(import_react32.default.Fragment, null, /* @__PURE__ */ import_react32.default.createElement("ul", { className: "aidos-evidence-list" }, records.map((row) => /* @__PURE__ */ import_react32.default.createElement(
    EvidenceStrip,
    {
  ...
  ))), comments.map((comment, position) => /* @__PURE__ */ import_react32.default.createElement("div", { className: "aidos-comment", key: position }, /* @__PURE__ */ import_react32.default.createElement("div", null, /* @__PURE__ */ import_react32.default.createElement("span", { className: "aidos-evidence-author" }, comment.author)), /* @__PURE__ */ import_react32.default.createElement("p", { className: "aidos-detail-body" }, comment.body))), evidenceViewer.viewer);
  return /* @__PURE__ */ import_react32.default.createElement(
    AidosRow,
    {
      icon: /* @__PURE__ */ import_react32.default.createElement(CompassIcon, null),
...
function useEvidenceViewer() {
  const [viewed, setViewed] = import_react32.default.useState(null);
  return {
    open: setViewed,
    viewer: viewed === null ? null : /* @__PURE__ */ import_react32.default.createElement(
      EvidenceViewer,
      {
        row: viewed,
        onClose: () => {
    ...
    try {
      ...
      return import_react33.default.createElement(
        ...
        import_react33.default.createElement(
          ...
          import_react33.default.createElement("span", { className: "tool-render-title" }, key),
          import_react33.default.createElement("span", { className: "tool-render-sep" }),
          import_react33.default.createElement(
            "span",
            { className: "tool-render-summary" },

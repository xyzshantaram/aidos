...
function ProjectionReader(props) {
  ...
  const selectedTicket = selectedKey === null ? null : rawTickets.find((ticket) => boardKeyOf(ticket) === selectedKey) ?? null;
  const selectedBoardKey = selectedTicket === null ? null : boardKeyOf(selectedTicket);
  const activeId = activeTicketId(rawTickets);
  const activeRow = activeId === null ? null : rawTickets.find((t) => t.id === activeId) ?? null;

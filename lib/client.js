...
function ProjectionReader(props) {
  ...
  const [merge, setMerge] = import_react16.default.useState(null);
  const mergeVersionRef = import_react16.default.useRef(-1);
  const ownVersion = ticketsProjection === void 0 ? -1 : JSON.stringify(ticketsProjection).length + ":" + Object.keys(ticketsProjection).length;
  import_react16.default.useEffect(function() {
    if (!loaded) return;
    let cancelled = false;
    const pull = async function() {
      try {
        ...
        if (!cancelled) setMerge(function(prev) {
          return prev;
        });
      }
    };

...
function ProjectionReader(props) {
  ...
  const resolvedTicket = selectedKey === null ? null : rawTickets.find((ticket) => boardKeyOf(ticket) === selectedKey) ?? null;
  ...
  const reanchored = resolvedTicket === null && previous !== null ? rawTickets.find(
  ...
  const selectedTicket = resolvedTicket ?? reanchored ?? (boardSettling ? previous : null);

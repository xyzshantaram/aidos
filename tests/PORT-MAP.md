...
| test_26_plan_round_trip | `test-26-plan-round-trip-live.test.ts` | Fixed in A-W4: the kernel-level `importPlan`/`exportPlan` in `src/plan/plan-io.ts` duplicated the live `plan_import`/`plan` tools and is deleted. The fixture now drives the live tools through the B1 harness. `ROUND_TRIP_TICKET_KEYS` = id, title, body, criteria, phase, order. Marks checked via `[${STATE_MARKS[state]}]`. Parse-error pins: the `plan_parse_error` refusal names the 1-based line, nothing imports. A non-empty project refuses with `project_not_empty`. |
...
| test_29_unknown_kind_never_tracebacks | `test-04-unregistered-kind-refused.test.ts` (kernel) and `test-26-plan-round-trip-live.test.ts` (plan import) | Fixed in A-W4: `tests/test-29-unknown-kind-never-tracebacks.test.ts` is deleted. Its `attach_evidence`-names-the-kind assertion duplicated `test-04`; its plan-import assertion (a kind-less config refuses `plan_import` with `unknown_kind` naming `builtin:imported_state`) now lives in `test-26-plan-round-trip-live.test.ts`. The no-traceback rendering is B1. |
